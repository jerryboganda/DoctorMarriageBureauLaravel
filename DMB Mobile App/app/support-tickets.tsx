...
import { View, Text, ScrollView, TouchableOpacity, Alert, ActivityIndicator, RefreshControl, TextInput, Modal, Image } from 'react-native';
...
import { 
  LifeBuoyIcon, ChevronLeftIcon, PlusIcon, MessageCircleIcon, ClockIcon,
  CheckCircleIcon, XCircleIcon, SendIcon, ImagePlusIcon, ChevronRightIcon, AlertCircleIcon
...
const SupportTicketsScreen = () => {
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      if (response.data.result) {
        ...
        setNewTicket({ subject: '', category: 'account', priority: 'medium', message: '', attachments: [] });
        fetchTickets();
      } else {
        Alert.alert(t('common.error'), response.data.message || t('support.createFailed'));
      }
    } catch (error: any) {
      Alert.alert(t('common.error'), error.response?.data?.message || t('support.createFailed'));
    } finally {
      setCreating(false);
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'open': return { bg: 'bg-blue-100', text: 'text-blue-600' };
      case 'in_progress': return { bg: 'bg-amber-100', text: 'text-amber-600' };
      case 'resolved': return { bg: 'bg-emerald-100', text: 'text-emerald-600' };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'open': return <AlertCircleIcon size={14} color="#3b82f6" />;
      case 'in_progress': return <ClockIcon size={14} color="#f59e0b" />;
      case 'resolved': return <CheckCircleIcon size={14} color="#22c55e" />;
      ...
      <View className="flex-1 bg-slate-50">
        ...
        >
          ...
          >
            ...
            <Text className="text-white/90 mt-4 font-medium">{t('support.loading')}</Text>
          </MotiView>
        </LinearGradient>
      </View>
    );
      ...
      >
        ...
        <View className="flex-row justify-center gap-8 pb-6">
          <View className="items-center">
            <Text className="text-3xl font-bold text-white">{tickets.filter(t => t.status === 'open').length}</Text>
            <Text className="text-white/70 text-xs">{t('support.open')}</Text>
          </View>
          <View className="w-px bg-white/30" />
          <View className="items-center">
            <Text className="text-3xl font-bold text-white">{tickets.filter(t => t.status === 'in_progress').length}</Text>
            <Text className="text-white/70 text-xs">{t('support.inProgress')}</Text>
          </View>
          <View className="w-px bg-white/30" />
          <View className="items-center">
            <Text className="text-3xl font-bold text-white">{tickets.filter(t => t.status === 'resolved').length}</Text>
      ...
      >
        {tickets.length === 0 ? (
          ...
          >
            ...
            <Text className="text-xl font-bold text-slate-900 mb-2">{t('support.noTickets')}</Text>
            ...
            >
              ...
              <Text className="text-white font-bold ml-2">{t('support.newTicket')}</Text>
            </TouchableOpacity>
          </MotiView>
        ) : (
                    ...
                    <View className="flex-row items-start justify-between">
                      <View className="flex-1">
                        <View className="flex-row items-center gap-2 mb-1">
                          <View className={`px-2 py-0.5 rounded-full flex-row items-center ${statusStyle.bg}`}>
                            {getStatusIcon(ticket.status)}
                            <Text className={`text-xs font-semibold ml-1 capitalize ${statusStyle.text}`}>
                              {ticket.status.replace('_', ' ')}
                            </Text>
                          </View>
                          {ticket.unread_count > 0 && (
                            <View className="bg-red-500 px-2 py-0.5 rounded-full">
                              <Text className="text-white text-xs font-bold">{ticket.unread_count}</Text>
                            </View>
                          )}
                        </View>
                        ...
                        <View className="flex-row items-center mt-2">
                          <Text className="text-slate-400 text-xs">#{ticket.ticket_id}</Text>
                          <Text className="text-slate-300 mx-2">•</Text>
                          <Text className="text-slate-400 text-xs">{formatDate(ticket.updated_at)}</Text>
                        </View>
                      </View>
        ...
        <View className="flex-1 bg-black/50 justify-end">
          ...
          >
            ...
            <ScrollView className="px-6" showsVerticalScrollIndicator={false}>
              <Text className="text-2xl font-bold text-slate-900 mb-6">{t('support.newSupportTicket')}</Text>

              {/* Subject */}
              <View className="mb-4">
                <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('support.subject')}</Text>
                <TextInput
                  className="bg-slate-100 rounded-xl px-4 py-3 text-base text-slate-900"
                  value={newTicket.subject}
                  onChangeText={(v) => setNewTicket(prev => ({ ...prev, subject: v }))}
              ...
              {/* Category */}
              <View className="mb-4">
                <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('support.category')}</Text>
                  ...
                  {categories.map((cat) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setNewTicket(prev => ({ ...prev, category: cat.value }));
                      }}
                    ...
                    >
                      <Text className={newTicket.category === cat.value ? 'text-white font-semibold' : 'text-slate-600'}>
                        {cat.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
              ...
              {/* Priority */}
              <View className="mb-4">
                <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('support.priority')}</Text>
                <View className="flex-row gap-2">
                  {priorities.map((pri) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        Haptics.selectionAsync();
                        setNewTicket(prev => ({ ...prev, priority: pri.value }));
                      }}
                      ...
                      style={newTicket.priority === pri.value ? { borderColor: pri.color, backgroundColor: pri.color + '10' } : {}}
                    >
                      <Text style={{ color: newTicket.priority === pri.value ? pri.color : '#64748b' }} className="font-semibold">
                        {pri.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
              ...
              {/* Message */}
              <View className="mb-4">
                <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('support.describeYourIssue')}</Text>
                <TextInput
                  className="bg-slate-100 rounded-xl px-4 py-3 text-base text-slate-900 min-h-[120px]"
                  value={newTicket.message}
                  onChangeText={(v) => setNewTicket(prev => ({ ...prev, message: v }))}
              ...
              {/* Attachments */}
              <View className="mb-6">
                <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('support.attachments')}</Text>
                <View className="flex-row flex-wrap gap-2">
                  {newTicket.attachments.map((uri, idx) => (
                    <Image key={idx} source={{ uri }} className="w-16 h-16 rounded-lg" />
                  ))}
                  <TouchableOpacity
                ...
                >
                  {creating ? (
                    ...
                    <>
                      ...
                      <Text className="text-white font-bold text-lg ml-2">{t('common.submit')}</Text>
                    </>
                  )}
                </TouchableOpacity>
              </View>
