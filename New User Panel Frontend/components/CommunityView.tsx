...
const CommunityView: React.FC = () => {
    ...
    const filteredCommunities = useMemo(() => {
        if (filter === 'joined')
            return communities.filter((community) => community.status === 'joined');
        if (filter === 'pending')
            return communities.filter((community) => community.status === 'pending');
        return communities;
    }, [communities, filter]);
            ...
            <div className="bg-white border-b border-slate-200 p-8 pb-0">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center gap-4 border-b border-slate-200">
                        <TabItem
                            label={t('community.discover')}
                            active={filter === 'all'}
                            onClick={() => setFilter('all')}
                        />
                        <TabItem
                            label={t('community.myCommunities')}
                            active={filter === 'joined'}
                            onClick={() => setFilter('joined')}
                            count={joinedCount}
                        />
                        <TabItem
                            label={t('community.pendingRequests')}
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between mb-4">
                                            <div
                                                className={`size-12 rounded-xl flex items-center justify-center border ${meta.color}`}
                                            >
                                                {meta.icon}
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2 mb-4">
                                            <span className="text-xs font-bold text-slate-500 px-2 py-0.5 bg-slate-100 rounded-md uppercase">
                                                {community.type}
                                            </span>
                                            <span className="text-xs text-slate-400">
                                                - {memberLabel} {t('community.members')}
                                            </span>
                                        </div>

                                        <p className="text-sm text-slate-600 mb-6 line-clamp-2 h-10">
                                            {community.description ||
                                                t('community.descriptionComingSoon')}
                                        </p>
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                                            ...
                                            {isJoined ? (
                                                ...
                                                <button
                                                    ...
                                                    disabled={
                                                        actionLoading[community.id] || isPending
                                                    }
                                                >
                                                    {actionLoading[community.id]
                ...
                <div className="fixed bottom-4 right-4 bg-green-600 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 z-50 animate-in slide-in-from-bottom-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-green-100">
                            {t('community.createdSuccessDesc')}
                        </p>
                    </div>
                </div>
            )}
            ...
            >
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-200">
                    <div>
                        <h3 className="text-xl font-bold text-slate-900">
                            {t('community.createModalTitle')}
                        </h3>
                        <p className="text-sm text-slate-500">
                            {t('community.createModalSubtitle')}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
                    ...
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-bold text-sm text-slate-900">
                                    {isPrivate
                                        ? t('community.privateCommunity')
                                        : t('community.publicCommunity')}
                                </p>
                        ...
                        >
                            <div
                                className={`size-5 bg-white rounded-full shadow transition-transform ${isPrivate ? 'translate-x-6' : 'translate-x-0.5'}`}
                            />
                        </button>
                    </div>
