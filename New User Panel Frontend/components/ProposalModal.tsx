...
const ProposalModal: React.FC<ProposalModalProps> = ({
    ...
    const [alreadySent, setAlreadySent] = useState<'pending' | 'accepted' | 'received' | null>(
        null,
    );
    ...
    const membershipLabel =
        user?.membership === 2 || user?.membership === '2'
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl p-12 text-center animate-in zoom-in-95 duration-200">
                    ...
                    <h3 className="text-xl font-bold text-slate-900">
                        {t('modals.proposal.sent')}
                    </h3>
                    <p className="text-slate-500 mt-2">
                        {t('modals.proposal.connecting', { name: profile.name })}
                    </p>
                </div>
            </div>
        );
                    ...
                    <div className="p-8 text-center">
                        ...
                        <p className="text-slate-500 text-sm mb-6">
                            {isReceived
                                ...
                                : isAccepted
                                  ? t('modals.proposal.proposalAcceptedDesc', {
                                        name: profile.name,
                                    })
                ...
                <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-slate-50">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            {t('modals.proposal.sendProposal')}
                        </h2>
                        <p className="text-xs text-slate-500">
                            {t('modals.proposal.to', { name: profile.name })}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <div className="bg-primary/5 border border-primary/10 rounded-xl p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {profile.matchReasons?.map((reason, idx) => (
                                <span
                                    key={idx}
                                    className="px-2 py-1 bg-white border border-primary/20 rounded-md text-xs font-medium text-primary-hover shadow-sm"
                                >
                                    {reason}
                                </span>
                            ))}
                            <span className="px-2 py-1 bg-white border border-primary/20 rounded-md text-xs font-medium text-primary-hover shadow-sm">
                                ...
                                {t('modals.proposal.compatibility')}
                            </span>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-xs">
                                ...
                                <p className="text-slate-500">
                                    {membershipLabel} - {verificationLabel}
                                </p>
                            </div>
                        </div>
                        <button
