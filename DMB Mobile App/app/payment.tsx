...
const PaymentScreen = () => {
    ...
    const fetchPackages = async () => {
        try {
            ...
            if (response.data.result) {
                ...
                if (params.packageId) {
                    const preSelected = packagesData.find(
                        (p: Package) => p.id === parseInt(params.packageId as string),
                    );
    ...
    const fetchWalletBalance = async () => {
        try {
            ...
            if (response.data.result) {
                setWalletBalance(
                    parseFloat(response.data.data?.balance || response.data.balance) || 0,
                );
            }
        } catch (error) {
    ...
    const applyCoupon = async () => {
        ...
        try {
            ...
            if (response.data.result) {
                ...
                Alert.alert(
                    t('common.success'),
                    `${t('payment.couponApplied')} PKR ${response.data.data.discount}`,
                );
            } else {
                Alert.alert(
                    t('payment.invalidCoupon'),
                    response.data.message || t('payment.couponNotValid'),
                );
            }
        } catch (error: any) {
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('payment.couponError'),
            );
        } finally {
        ...
        try {
            ...
            if (response.data.result) {
                ...
                if (response.data.data.payment_url) {
                    ...
                    Alert.alert(t('payment.completePayment'), t('payment.redirectPayment'), [
            ...
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('payment.processingFailed'),
            );
        } finally {
            setProcessing(false);
        }
    ...
    const getFeatureIcon = (feature: string) => {
        ...
        if (lower.includes('interest') || lower.includes('express'))
            return <HeartIcon size={16} color="#ef4444" />;
        if (lower.includes('message') || lower.includes('chat'))
            return <MessageCircleIcon size={16} color="#10b981" />;
        if (lower.includes('shortlist')) return <BookmarkIcon size={16} color="#f59e0b" />;
        if (lower.includes('highlight') || lower.includes('boost'))
            return <SparklesIcon size={16} color="#8b5cf6" />;
        if (lower.includes('support') || lower.includes('priority'))
            return <ShieldIcon size={16} color="#06b6d4" />;
        ...
        <View className="flex-1 bg-slate-50">
            ...
            <ScrollView className="flex-1 px-4 -mt-4" showsVerticalScrollIndicator={false}>
                ...
                {packages.map((pkg, index) => (
                    ...
                    >
                        ...
                        >
                            <LinearGradient
                                colors={
                                    pkg.is_popular ? ['#7c3aed', '#a855f7'] : ['#ffffff', '#f8fafc']
                                }
                                start={{ x: 0, y: 0 }}
                                end={{ x: 1, y: 1 }}
                                className="p-4"
                            >
                                ...
                                <View className="flex-row items-center justify-between mb-3">
                                    <View className="flex-row items-center">
                                        ...
                                        <View className="ml-3">
                                            <Text
                                                className={`text-lg font-bold ${pkg.is_popular ? 'text-white' : 'text-slate-900'}`}
                                            >
                                                {pkg.name}
                                            </Text>
                                            <Text
                                                className={`text-xs ${pkg.is_popular ? 'text-white/70' : 'text-slate-500'}`}
                                            >
                                    ...
                                    <View className="items-end">
                                        {pkg.original_price && (
                                            <Text
                                                className={`text-sm line-through ${pkg.is_popular ? 'text-white/50' : 'text-slate-400'}`}
                                            >
                                                PKR {pkg.original_price.toLocaleString()}
                                            </Text>
                                        )}
                                        <Text
                                            className={`text-xl font-bold ${pkg.is_popular ? 'text-white' : 'text-violet-600'}`}
                                        >
                                ...
                                <View className="border-t border-slate-200/20 pt-3">
                                    {pkg.features.slice(0, 4).map((feature, idx) => (
                                        <View key={idx} className="flex-row items-center mb-2">
                                            ...
                                            <Text
                                                className={`ml-2 text-sm ${pkg.is_popular ? 'text-white/90' : 'text-slate-600'}`}
                                            >
                                                {feature}
                                            </Text>
                                        </View>
                                    ...
                                    {pkg.features.length > 4 && (
                                        <Text
                                            className={`text-xs mt-1 ${pkg.is_popular ? 'text-white/70' : 'text-slate-500'}`}
                                        >
                ...
                >
                    ...
                    <View className="flex-row items-center">
                        <TextInput
                            className={`flex-1 bg-slate-100 rounded-xl px-4 py-3 text-base ${
                                couponApplied ? 'bg-emerald-50' : ''
                            }`}
                            value={couponCode}
