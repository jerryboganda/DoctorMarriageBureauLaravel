...
import {
  ChevronLeftIcon, HeartIcon, CheckIcon, XIcon, UserIcon,
  MessageCircleIcon, BriefcaseIcon, MapPinIcon, CalendarIcon, SendIcon
...
const InterestsScreen = () => {
  ...
  const fetchInterests = async () => {
    try {
      ...
      if (receivedRes.data.result) {
        setReceivedInterests(receivedRes.data.data.map((i: any) => ({
          id: i.id,
          type: 'received',
      ...
      if (sentRes.data.result) {
        setSentInterests(sentRes.data.data.map((i: any) => ({
          id: i.id,
          type: 'sent',
          status: i.status?.toLowerCase() === 'approved' ? 'accepted' : 'pending',
          created_at: i.created_at || new Date().toISOString(),
  ...
  const handleAccept = async (interest: Interest) => {
    ...
    try {
      const response = await api.post('/member/interest-accept', { interest_id: interest.id });
      ...
      Alert.alert(t('common.error'), error.response?.data?.message || t('interests.acceptFailed'));
    } finally {
  ...
  const handleReject = async (interest: Interest) => {
    Alert.alert(
      t('interests.declineTitle'),
      t('interests.declineConfirm'),
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            setProcessingId(interest.id);
            try {
              const response = await api.post('/member/interest-reject', { interest_id: interest.id });
              if (response.data.result) {
                setReceivedInterests(prev =>
              ...
              Alert.alert(t('common.error'), error.response?.data?.message || t('interests.declineFailed'));
            } finally {
              setProcessingId(null);
            }
          }
  ...
  const handleCancelSent = async (interest: Interest) => {
    Alert.alert(
      t('interests.cancelTitle'),
      t('interests.cancelConfirm'),
      [
        { text: t('common.no'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            setProcessingId(interest.id);
            try {
              const response = await api.post('/member/interest-withdraw', { interest_id: interest.id });
              if (response.data.result) {
                setSentInterests(prev => prev.filter(i => i.id !== interest.id));
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              }
            } catch (error: any) {
              Alert.alert(t('common.error'), error.response?.data?.message || t('interests.cancelFailed'));
            } finally {
              setProcessingId(null);
            }
          }
      ...
      case 'accepted':
        return (
          <View className="bg-emerald-100 px-3 py-1 rounded-full flex-row items-center">
            ...
            <Text className="text-emerald-600 text-xs font-semibold ml-1">{t('interests.accepted')}</Text>
          </View>
        );
      case 'rejected':
        return (
          <View className="bg-red-100 px-3 py-1 rounded-full flex-row items-center">
            ...
            <Text className="text-red-600 text-xs font-semibold ml-1">{t('interests.declined')}</Text>
          </View>
        );
      default:
        return (
          <View className="bg-amber-100 px-3 py-1 rounded-full">
            <Text className="text-amber-600 text-xs font-semibold">{t('interests.pending')}</Text>
          </View>
        );
    }
  };
      ...
      >
          ...
          <View className="flex-row items-center">
            ...
            {pendingCount > 0 && (
              <View className="ml-2 bg-white px-2 py-0.5 rounded-full">
                <Text className="text-red-600 text-xs font-bold">{pendingCount}</Text>
              </View>
            )}
          </View>
        ...
        <View className="flex-row px-4 pb-4">
          <TouchableOpacity
            ...
            className={`flex-1 py-3 rounded-xl mr-2 flex-row items-center justify-center ${activeTab === 'received' ? 'bg-white' : 'bg-white/20'
              }`}
          >
            <HeartIcon size={18} color={activeTab === 'received' ? '#ef4444' : 'white'} />
            <Text className={`ml-2 font-semibold ${activeTab === 'received' ? 'text-red-600' : 'text-white'
              }`}>
          ...
          <TouchableOpacity
            ...
            className={`flex-1 py-3 rounded-xl flex-row items-center justify-center ${activeTab === 'sent' ? 'bg-white' : 'bg-white/20'
              }`}
          >
            <SendIcon size={18} color={activeTab === 'sent' ? '#ef4444' : 'white'} />
            <Text className={`ml-2 font-semibold ${activeTab === 'sent' ? 'text-red-600' : 'text-white'
              }`}>
        ...
        {currentInterests.length === 0 ? (
          ...
          >
            ...
            <Text className="text-xl font-bold text-slate-900 mb-2">
              {t(activeTab === 'received' ? 'interests.noReceived' : 'interests.noSent')}
            </Text>
            ...
            >
              <Text className="text-white font-semibold">{t('interests.exploreProfiles')}</Text>
            </TouchableOpacity>
          </MotiView>
        ) : (
              ...
              <View className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <View className="relative">
                    {interest.user.photo ? (
                      <Image
                        source={{ uri: getProfileImageUrl(interest.user.photo) }}
                  ...
                  <View className="flex-1 ml-4">
                    ...
                    {interest.user.age && (
                      <View className="flex-row items-center mb-1">
                        ...
                        <Text className="text-slate-600 text-sm ml-1">{interest.user.age} years old</Text>
                      </View>
                    )}
                    ...
                    {(interest.user.city || interest.user.country) && (
                      <View className="flex-row items-center">
                        ...
                        <Text className="text-slate-600 text-sm ml-1">
                          {[interest.user.city, interest.user.country].filter(Boolean).join(', ')}
                        </Text>
                      </View>
                    )}
                ...
                {interest.message && (
                  <View className="px-4 pb-3">
                    <View className="bg-slate-50 rounded-lg p-3">
                      <Text className="text-slate-600 text-sm italic">"{interest.message}"</Text>
                    </View>
                  </View>
                )}
                ...
                {activeTab === 'received' && interest.status === 'pending' && (
                  <View className="flex-row border-t border-slate-100">
                    ...
                    >
                      ...
                      <Text className="text-red-500 font-semibold ml-2">{t('interests.decline')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      {processingId === interest.id ? (
                        ...
                        <>
                          ...
                          <Text className="text-emerald-500 font-semibold ml-2">{t('interests.accept')}</Text>
                        </>
                      )}
                    </TouchableOpacity>
                  </View>
                ...
                {interest.status === 'accepted' && (
                  ...
                  >
                    ...
                    <Text className="text-emerald-600 font-semibold ml-2">{t('interests.startConversation')}</Text>
                  </TouchableOpacity>
                )}
                ...
                {activeTab === 'sent' && interest.status === 'pending' && (
                  ...
                  >
                    {processingId === interest.id ? (
                      ...
                      <>
                        <XIcon size={18} color="#ef4444" />
                        <Text className="text-red-500 font-semibold ml-2">{t('interests.cancelInterest')}</Text>
                      </>
                    )}
                  </TouchableOpacity>
                )}
