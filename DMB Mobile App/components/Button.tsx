...
const Button: React.FC<ButtonProps> = ({
  ...
  const baseContainerStyle = "h-14 rounded-2xl flex-row items-center justify-center overflow-hidden";
  ...
  return (
    <AnimatedPressable
      ...
      className={cn(baseContainerStyle, variants[variant], fullWidth && "w-full", className, (actuallyLoading || disabled) && "opacity-80")}
    ...
    >
      {actuallyLoading ? (
        <View className="flex-row items-center gap-2">
          <ActivityIndicator color={variant === 'primary' || variant === 'danger' ? 'white' : '#334155'} />
