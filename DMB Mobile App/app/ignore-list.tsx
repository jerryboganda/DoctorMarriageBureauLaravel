...
const IgnoreListScreen = () => {
    ...
    const handleRemove = async (item: IgnoredUser) => {
        ...
        try {
            const response = await api.post('/member/remove-from-ignored-list', {
                user_id: item.ignored_user.id,
            });
            if (response.data.result) {
                ...
                Alert.alert(
                    t('common.error'),
                    response.data.message || t('ignoreList.removeFailed'),
                );
            }
        } catch (error: any) {
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('ignoreList.removeFailed'),
            );
        } finally {
            setRemovingId(null);
        }
    ...
    const clearAll = () => {
        ...
        Alert.alert(t('ignoreList.clearTitle'), t('ignoreList.clearConfirm'), [
                ...
                onPress: async () => {
                    try {
                        ...
                        for (const item of ignoredUsers) {
                            await api.post('/member/remove-from-ignored-list', {
                                user_id: item.ignored_user.id,
                            });
                        }
                        setIgnoredUsers([]);
                ...
                <View className="items-center pb-6">
                    ...
                    <View className="mt-3 bg-white/20 px-4 py-2 rounded-full">
                        <Text className="text-white font-semibold">
                            {t('ignoreList.ignoredCount', { count: ignoredUsers.length })}
                        </Text>
                    </View>
                </View>
            </LinearGradient>
            ...
            >
                ...
                >
                    ...
                    <View className="flex-1 ml-3">
                        <Text className="text-indigo-800 font-semibold mb-1">
                            {t('ignoreList.ignoreVsBlock')}
                        </Text>
                            ...
                            <View className="bg-white rounded-2xl shadow-sm overflow-hidden">
                                ...
                                >
                                    ...
                                    {item.ignored_user.photo ? (
                                        <Image
                                            source={{
                                                uri: getProfileImageUrl(item.ignored_user.photo),
                                            }}
                                    ...
                                    <View className="flex-1 ml-4 justify-center">
                                        <Text className="text-lg font-bold text-slate-900">
                                            {item.ignored_user.first_name}{' '}
                                            {item.ignored_user.last_name}
                                        </Text>
