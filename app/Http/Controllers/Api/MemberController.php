...
class MemberController extends Controller
{
    ...
    {
        $label = is_string($element)
            ? $element
            : (is_array($element) ? ($element['label'] ?? '') : ($element->label ?? ''));
    ...
    public function store_verification_info(Request $request)
    {
        ...
        try {
            ...
            $formFields = json_decode($setting->value, true);
            ...
            foreach ($formFields as $key => $element) {
                ...
                if ($type == 'text') {
                    ...
                    if (! $file) {
                        if ($this->isOptionalVerificationField($label)) {
                            $item['value'] = '';
                        ...
                        return response()->json([
                            ...
                            'message' => translate('Please upload the required document: :label', ['label' => $label]),
                        ]);
                    }
                    if (! $file->isValid()) {
                        return response()->json([
                            ...
                            'message' => translate('The uploaded file for :label is invalid or exceeded upload limits. Please retry with a smaller file.', ['label' => $label]),
                        ], 422);
                    }
                    $storedPath = $file->store('uploads/verification_form');
                    if (! $storedPath) {
                        return response()->json([
                            'result' => false,
                            'error_code' => 'document_storage_failed',
                            'message' => translate('The uploaded document could not be saved. Please try again.'),
                        ], 500);
                    }
                    $item['value'] = $storedPath;
                }
                array_push($data, $item);
                $i++;
            }
