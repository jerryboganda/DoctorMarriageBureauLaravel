...
const PhotoGalleryScreen = () => {
    ...
    const pickImage = async (useCamera: boolean = false) => {
        if (photos.length >= maxPhotos) {
            Alert.alert(
                t('photoGallery.limitReached'),
                t('photoGallery.limitReachedDesc', { max: maxPhotos }),
            );
            return;
        }
        ...
        if (!permissionResult.granted) {
            Alert.alert(
                ...
                t('photoGallery.grantAccess', {
                    type: useCamera ? t('common.camera') : t('common.gallery'),
                }),
            );
            return;
        }
    ...
    const uploadPhoto = async (image: any) => {
        ...
        try {
            ...
            if (response.data.result) {
                ...
                Alert.alert(
                    t('common.error'),
                    response.data.message || t('photoGallery.uploadFailed'),
                );
            }
        } catch (error: any) {
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('photoGallery.uploadFailed'),
            );
        } finally {
            setUploading(false);
        }
    ...
    const handleSetPrimary = async (photo: GalleryPhoto) => {
        ...
        try {
            ...
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('photoGallery.setPrimaryFailed'),
            );
        }
    };
    ...
    const handleTogglePrivate = async (photo: GalleryPhoto) => {
        try {
            ...
            if (response.data.result) {
                setPhotos((prev) =>
                    prev.map((p) => (p.id === photo.id ? { ...p, is_private: !p.is_private } : p)),
                );
                setSelectedPhoto((prev) =>
                    prev ? { ...prev, is_private: !prev.is_private } : null,
                );
            ...
            Alert.alert(
                t('common.error'),
                error.response?.data?.message || t('photoGallery.privacyFailed'),
            );
        }
    };
    ...
    const handleDeletePhoto = (photo: GalleryPhoto) => {
        ...
        Alert.alert('Delete Photo', t('photoGallery.deleteConfirm'), [
            { text: t('common.cancel'), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert(
                            t('common.error'),
                            error.response?.data?.message || t('photoGallery.deleteFailed'),
                        );
                    }
                },
            },
                    ...
                    >
                        {photos.map((photo, index) => (
                            ...
                            >
                                <Image
                                    source={{
                                        uri: getProfileImageUrl(photo.thumbnail || photo.url),
                                    }}
