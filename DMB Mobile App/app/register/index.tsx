...
import { MailIcon, SmartphoneIcon, CaduceusIcon, ChevronRightIcon, LockIcon, UserIcon, CalendarIcon, PhoneIcon, GoogleIcon } from '../../components/Icons';
...
export default function RegisterFlow() {
  ...
  const handleGoogleLogin = async (token: string | undefined) => {
    ...
    try {
      ...
      if (access_token && user) {
        ...
        Alert.alert(t('auth.register.errors.regFailedTitle'), t('auth.register.errors.regFailedMsg'));
      }
    } catch (err: any) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      console.error('Google Registration Error:', err);
      Alert.alert(t('auth.register.errors.googleRegError'), err.response?.data?.message || t('auth.register.errors.googleRegFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendOtp = async () => {
    if (!identifier) {
      setError(method === 'email' ? t('auth.register.errors.enterEmail') : t('auth.register.errors.enterPhone'));
      return;
    }
    ...
    setError('');
    try {
      const endpoint = method === 'email' ? '/send-email-verification' : '/send-phone-verification';
      const payload = method === 'email' ? { email: identifier, intent: 'signup' } : { phone: identifier, intent: 'signup' };
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
          <View className="flex-1 px-6 justify-center" style={{ paddingTop: insets.top + 20, paddingBottom: 40 }}>
            ...
            >
              <ChevronRightIcon size={20} color="#1e293b" style={{ transform: [{ rotate: '180deg' }] }} />
            </TouchableOpacity>
            ...
            >
              ...
              <View className="items-center mb-8">
                ...
                <Text className="text-2xl font-bold text-slate-900 text-center tracking-tight">
                  ...
                  {step === 'input' && (method === 'email' ? t('auth.register.professionalEmail') : t('auth.register.phoneNumber'))}
              ...
              {error ? (
                <MotiView from={{ opacity: 0, translateY: -10 }} animate={{ opacity: 1, translateY: 0 }} className="bg-red-50 p-4 rounded-2xl mb-6 border border-red-100">
                ...
                <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'timing', duration: 300 }}>
                  ...
                  >
                    ...
                    <View className="flex-1">
                      <Text className="text-lg font-bold text-slate-900">{t('auth.register.emailAddress')}</Text>
                      <Text className="text-slate-500 text-xs font-medium uppercase tracking-wider">{t('auth.register.mostProfessional')}</Text>
                    </View>
                  ...
                  >
                    ...
                    <View className="flex-1">
                      <Text className="text-lg font-bold text-slate-900">{t('auth.register.mobilePhone')}</Text>
                      <Text className="text-slate-500 text-xs font-medium uppercase tracking-wider">{t('auth.register.fastestAccess')}</Text>
                    </View>
                    <ChevronRightIcon size={20} color="#94a3b8" />
                  </TouchableOpacity>
                  ...
                  <View className="flex-row items-center my-6">
                    ...
                    <Text className="mx-4 text-slate-300 font-bold text-[10px] uppercase tracking-widest">{t('auth.register.or')}</Text>
                    <View className="flex-1 h-[1px] bg-slate-100" />
                  </View>
                  ...
                  >
                    <GoogleIcon size={20} />
                    <Text className="ml-3 text-slate-700 font-bold text-base">{t('auth.register.signUpWithGoogle')}</Text>
                  </TouchableOpacity>
                </MotiView>
              )}
              ...
              {step === 'input' && (
                <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'timing', duration: 300 }}>
                  <Input
                    label={method === 'email' ? t('auth.register.professionalEmail') : t('auth.register.phoneNumber')}
                    icon={method === 'email' ? <MailIcon size={20} color="#64748b" /> : <SmartphoneIcon size={20} color="#64748b" />}
                    keyboardType={method === 'email' ? 'email-address' : 'phone-pad'}
                    autoCapitalize="none"
                    value={identifier}
                    onChangeText={setIdentifier}
                    placeholder={method === 'email' ? 'e.g. name@hospital.com' : 'e.g. +1 555 0123'}
                  />
                  <Button variant="primary" className="mt-6 py-5 rounded-2xl" onPress={handleSendOtp} isLoading={loading}>
              ...
              {step === 'otp' && (
                <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'timing', duration: 300 }}>
                  ...
                  <Button variant="primary" className="py-5 rounded-2xl" onPress={handleVerifyOtp} isLoading={loading}>
                  ...
                  <TouchableOpacity className="mt-6" onPress={handleSendOtp}>
                    <Text className="text-slate-400 text-center font-medium">
                      {t('auth.register.didntReceive')} <Text className="text-blue-600 font-bold">{t('auth.register.resendCode')}</Text>
                    </Text>
                  </TouchableOpacity>
                </MotiView>
              )}
              ...
              {step === 'personal' && (
                <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'timing', duration: 300 }}>
                  ...
                  <View className="mb-6">
                    <Text className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 ml-1">{t('auth.register.identifyAs')}</Text>
                    <View className="flex-row gap-3">
                      {[{ value: 'Male', label: t('auth.register.male') }, { value: 'Female', label: t('auth.register.female') }].map((g) => (
                        <TouchableOpacity
                          key={g.value}
                          onPress={() => setFormData({ ...formData, gender: g.value })}
                          activeOpacity={0.8}
                          className={`flex-1 py-4 rounded-2xl border-2 items-center justify-center ${formData.gender === g.value ? 'border-blue-600 bg-blue-50' : 'border-slate-100 bg-slate-50'
                            }`}
                        >
                          <Text className={`text-base ${formData.gender === g.value ? 'text-blue-700 font-bold' : 'text-slate-500 font-medium'}`}>{g.label}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </View>
              ...
              {step === 'account' && (
                <MotiView from={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: 'timing', duration: 300 }}>
                  ...
                  <Button variant="primary" className="mt-8 py-5 rounded-2xl shadow-lg shadow-blue-500/30" onPress={handleRegister} isLoading={loading}>
            ...
            <DatePicker
              ...
              initialDate={formData.date_of_birth ? new Date(formData.date_of_birth) : null}
            />
            ...
            {step === 'method' && (
              ...
              >
                <TouchableOpacity onPress={() => router.replace('/login')} className="flex-row items-center py-2">
                  <Text className="text-slate-500 text-base">{t('auth.register.alreadyMember')} </Text>
                  <Text className="text-blue-600 font-bold text-base">{t('auth.register.signIn')}</Text>
                </TouchableOpacity>
              </MotiView>
            )}
          </View>
