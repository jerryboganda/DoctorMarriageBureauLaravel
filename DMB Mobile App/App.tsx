...
import { EyeIcon, EyeOffIcon, MailIcon, LockIcon, GoogleIcon, AppleIcon, CaduceusIcon } from './components/Icons';
      ...
      <div className="w-full h-full flex flex-col relative overflow-hidden bg-slate-50">
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'login' && (
            ...
            >
              ...
              <div className="flex-1 min-h-[120px] flex items-center justify-center flex-col mt-4">
                   ...
                   >
                      <img src="/assets/images/logo.png" className="w-full h-full object-contain drop-shadow-lg" alt="Doctor Marriage Bureau" />
                   </motion.div>
                   ...
                   <motion.p variants={itemVariants} className="text-slate-500 text-center mt-4 text-sm font-medium">
              ...
              <div className="flex-shrink-0 pb-[max(3rem,env(safe-area-inset-bottom))]">
                  ...
                  <motion.div variants={itemVariants} className="mt-8 flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                       ...
                       <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Or Verify With</span>
                       <div className="h-px bg-slate-200 flex-1" />
                    </div>
                  ...
                  <motion.p variants={itemVariants} className="mt-8 text-center text-sm text-slate-500 font-medium">
                      New Doctor? <button onClick={() => setView('register')} className="text-brand-red font-bold cursor-pointer active:opacity-60 transition-all">Apply for Membership</button>
                  </motion.p>
              </div>
            </motion.div>
          )}
          ...
          {view === 'register' && (
             ...
             >
                <RegistrationFlow onBack={() => setView('login')} onSuccess={() => setView('dashboard')} />
             </motion.div>
          )}
          ...
          {view === 'forgot-password' && (
             <motion.div
               ...
               className="absolute inset-0 z-10 bg-slate-50"
             >
                <ForgotPasswordFlow onBack={() => setView('login')} onComplete={() => setView('login')} />
             </motion.div>
          )}
