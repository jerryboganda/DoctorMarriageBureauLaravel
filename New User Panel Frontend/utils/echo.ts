import Echo from 'laravel-echo';
import Pusher from 'pusher-js';

declare global {
    interface Window {
        Pusher: typeof Pusher;
        Echo: Echo;
    }
}

window.Pusher = Pusher;

const pusherHost = import.meta.env.VITE_PUSHER_HOST;
const pusherPort = import.meta.env.VITE_PUSHER_PORT;
const pusherScheme = import.meta.env.VITE_PUSHER_SCHEME;

export const echo = import.meta.env.VITE_PUSHER_APP_KEY ? new Echo({
    broadcaster: 'pusher',
    key: import.meta.env.VITE_PUSHER_APP_KEY,
    wsHost: pusherHost,
    wsPort: pusherPort ? parseInt(pusherPort) : 6001,
    wssPort: pusherPort ? parseInt(pusherPort) : 6001,
    forceTLS: pusherScheme === 'https',
    enabledTransports: ['ws', 'wss'],
    disableStats: true,
    cluster: import.meta.env.VITE_PUSHER_APP_CLUSTER || 'mt1',
    authEndpoint: (import.meta.env.VITE_API_BASE_URL || (import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace(/\/api$/, "") : "")) + "/broadcasting/auth",
    auth: {
        headers: {
            Authorization: 'Bearer ' + localStorage.getItem('auth_token'),
            'X-Requested-With': 'XMLHttpRequest',
        }
    }
}) : null;

if (!import.meta.env.VITE_PUSHER_APP_KEY) {
    console.warn('Pusher/Echo configuration missing. Real-time updates disabled.');
}

export default echo;
