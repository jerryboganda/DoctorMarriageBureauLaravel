...
class MemberUtility
{
    ...
    public static function member_interest_info($user_id = '')
    {
        ...
        if (empty($do_expressed_interest) && empty($received_expressed_interest)) {
            ...
            $data['interest_text'] = translate('Proposal');
        } elseif (!empty($received_expressed_interest)) {
            $data['interest_status'] = 'do_response';
            $data['interest_text'] = $received_expressed_interest->status == 0 ? translate('Reply to Proposal') : translate('You Accepted Proposal');
        } else {
            $data['interest_status'] = 0;
            $data['interest_text'] = $do_expressed_interest->status == 0 ? translate('Proposal Sent') : translate('Proposal Accepted');
        }
        return $data;
    }
