            ...
            <ul class="aiz-side-nav-list" id="main-menu" data-toggle="aiz-side-menu">
                ...
                <li class="aiz-side-nav-item">
                    ...
                    <ul class="aiz-side-nav-list level-2">
                        ...
                        @can('approve_member')
                        <li class="aiz-side-nav-item">
                            <a href="{{ route('verification_requests') }}" class="aiz-side-nav-link {{ areActiveRoutes(['verification_requests']) }}">
                                <span class="aiz-side-nav-text">{{ translate('Verification Requests') }}</span>
                                @php $pendingCount = \App\Models\User::where('user_type','member')->whereNotNull('verification_info')->where('approved',0)->count(); @endphp
                                @if($pendingCount > 0)
                                    <span class="badge badge-info badge-inline badge-md ml-auto">{{ $pendingCount }}</span>
                                @endif
                            </a>
                        </li>
                        @endcan
