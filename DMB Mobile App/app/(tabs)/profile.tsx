...
import { View, Text, ScrollView, TextInput, TouchableOpacity, Image, Alert, ActivityIndicator, Animated, Dimensions, Platform } from 'react-native';
...
import {
  UserIcon, BriefcaseIcon, HeartIcon, CameraIcon, EditIcon, CheckIcon, SettingsIcon, LogOutIcon,
  CoffeeIcon, HomeIcon, ImageIcon, DownloadIcon, HistoryIcon, AlertCircleIcon, SaveIcon,
  TrashIcon, UploadIcon, ImagePlusIcon, MicIcon, Volume2Icon, PlayCircleIcon, StarIcon, SparklesIcon
...
const FormSection = ({ title, icon: Icon, children, color = '#3b82f6' }: any) => (
  ...
  >
    <View className="flex-row items-center mb-4 px-1">
      <View style={{ backgroundColor: `${color}15` }} className="w-10 h-10 rounded-2xl items-center justify-center mr-3">
...
const FormSelect = ({ label, value, options, onChange, color = '#3b82f6' }: any) => (
  <View className="mb-5">
    <Text className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1 mb-2.5">{label}</Text>
    <View className="flex-row flex-wrap gap-2">
      {options.map((opt: any) => (
        <TouchableOpacity
          ...
          className={`px-4 py-2.5 rounded-2xl border ${value === opt.value
            ? 'bg-slate-900 border-slate-900'
            : 'bg-slate-50 border-slate-100'
            }`}
        >
          <Text className={`text-sm font-bold ${value === opt.value ? 'text-white' : 'text-slate-600'}`}>
            {opt.label}
          </Text>
        </TouchableOpacity>
      ))}
...
const ProfileTab = () => {
  ...
  const profileTabs = [
    ...
    { id: 'preferences', label: t('profile.tabs.preferences'), icon: HeartIcon, color: '#ef4444' },
      ...
      <View className="flex-1 bg-slate-50">
        ...
        >
          ...
          >
            ...
            <Text className="text-white/90 mt-4 font-medium">{t('profile.loading')}</Text>
          </MotiView>
        </LinearGradient>
      </View>
    );
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        ...
        <View className="flex-row justify-between items-center px-6 py-4">
          ...
          <Text className="text-white font-bold text-lg tracking-wide opacity-90">{t('profile.myProfile')}</Text>
          <TouchableOpacity
        ...
        <View className="items-center mt-2 mb-8">
          ...
          >
            ...
            <TouchableOpacity onPress={handleImagePick} disabled={uploading} activeOpacity={0.9}>
              {user?.avatar ? (
                <Image
          ...
          <View className="items-center mt-5 mb-6">
            <Text className="text-3xl font-bold text-white tracking-tight text-center shadow-sm">{user?.name || t('profile.yourName')}</Text>
            <Text className="text-blue-100 text-base font-medium mt-1 opacity-90">{user?.email}</Text>
          </View>
          ...
          <View className="flex-row items-center justify-center gap-3 mb-8 w-full px-4 flex-wrap">
            ...
            <View className="bg-amber-400/20 px-4 py-2 rounded-2xl flex-row items-center border border-amber-400/30 backdrop-blur-md">
              ...
              <Text className="text-amber-100 text-xs font-bold ml-2 uppercase tracking-wider">{t('profile.premiumMember')}</Text>
            </View>
            ...
            {qualityScore && (
              <View className="bg-emerald-400/20 px-4 py-2 rounded-2xl flex-row items-center border border-emerald-400/30 backdrop-blur-md">
                <ActivityIndicator size="small" color="#34d399" className={loading ? '' : 'hidden'} />
          ...
          <View className="flex-row justify-center gap-4 w-full px-6">
            ...
            >
              ...
              <Text className="text-white text-sm font-bold ml-2.5">{t('profile.downloadBiodata')}</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text className="text-white text-sm font-bold ml-2.5">{t('profile.profileViewers')}</Text>
            </TouchableOpacity>
          </View>
        </View>
            ...
            {profileTabs.map((tab) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center px-5 py-2.5 rounded-full mx-1.5 shadow-sm border ${isActive
                    ? 'bg-blue-600 border-blue-600 shadow-blue-200'
                    : 'bg-white border-slate-200 shadow-slate-100'
                    }`}
                >
                  <Icon size={16} color={isActive ? 'white' : '#64748b'} />
                  <Text className={`ml-2 font-bold text-sm ${isActive ? 'text-white' : 'text-slate-600'}`}>
                    {tab.label}
                  </Text>
                </TouchableOpacity>
              );
      ...
      {/* Content */}
      <ScrollView ref={scrollRef} className="flex-1 px-4 py-4 bg-white" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {activeTab === 'basics' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.basics.title')}</Text>
              </View>

              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.firstName')}</Text>
                <TextInput
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.lastName')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-semibold bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.bio')}</Text>
                <TextInput
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.dateOfBirth')}</Text>
                <TextInput
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.height')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">{t('profile.basics.city')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
          ...
          {activeTab === 'lifestyle' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.lifestyle.title')}</Text>
              </View>
              ...
              <FormSelect
                ...
                options={[
                  { value: 'vegetarian', label: t('profile.lifestyle.vegetarian') },
                  { value: 'non_vegetarian', label: t('profile.lifestyle.nonVegetarian') },
                  { value: 'vegan', label: t('profile.lifestyle.vegan') },
                  { value: 'eggetarian', label: t('profile.lifestyle.eggetarian') },
              ...
              <FormSelect
                ...
                options={[
                  { value: 'never', label: t('profile.lifestyle.never') },
                  { value: 'occasionally', label: t('profile.lifestyle.occasionally') },
                  { value: 'regularly', label: t('profile.lifestyle.regularly') }
                ]}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-purple-600 uppercase tracking-wider mb-2">{t('profile.lifestyle.hobbies')}</Text>
                <TextInput
                  className="text-slate-900 text-base bg-white rounded-xl px-4 py-3 border border-slate-200 min-h-[100px]"
                  multiline
                  textAlignVertical="top"
              ...
              <FormSelect
                ...
                options={[
                  { value: 'early_bird', label: t('profile.lifestyle.earlyBird') },
          ...
          {activeTab === 'career' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.career.title')}</Text>
              </View>

              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.highestEducation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={careerInfo.highest_education || careerInfo.education || ''}
                  onChangeText={(v) => updateField('career', 'highest_education', v)}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.university')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.profession')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.company')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <FormSelect
                ...
                options={[
                  ...
                  { value: 'self_employed', label: t('profile.career.selfEmployed') },
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.annualIncome')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">{t('profile.career.licenseNumber')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
          ...
          {activeTab === 'family' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.family.title')}</Text>
              </View>
              ...
              <FormSelect
                ...
                options={[
                  { value: 'traditional', label: t('profile.family.traditional') },
              ...
              <FormSelect
                ...
                options={[
                  ...
                  { value: 'upper_middle', label: t('profile.family.upperMiddle') },
                  { value: 'middle', label: t('profile.family.middleClass') },
                  { value: 'lower_middle', label: t('profile.family.lowerMiddle') }
                ]}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">{t('profile.family.fatherOccupation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={family.father_occupation || ''}
                  onChangeText={(v) => updateField('family', 'father_occupation', v)}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">{t('profile.family.motherOccupation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={family.mother_occupation || ''}
                  onChangeText={(v) => updateField('family', 'mother_occupation', v)}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">{t('profile.family.siblings')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">{t('profile.family.familyLocation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={family.family_location || ''}
                  onChangeText={(v) => updateField('family', 'family_location', v)}
          ...
          {activeTab === 'preferences' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.preferences.title')}</Text>
              </View>

              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-3">{t('profile.preferences.ageRange')}</Text>
                <View className="flex-row gap-3">
                  <View className="flex-1">
                    <Text className="text-xs text-slate-500 mb-1 text-center">{t('profile.preferences.minimum')}</Text>
                    <TextInput
                      ...
                      onChangeText={(v) => updateField('preferences', 'min_age', v)}
                      placeholder="25"
                      keyboardType="numeric"
                      placeholderTextColor="#94a3b8"
                    />
                  ...
                  <View className="items-center justify-end pb-3">
                    <Text className="text-slate-400 font-bold">{t('profile.preferences.to')}</Text>
                  </View>
                  <View className="flex-1">
                    <Text className="text-xs text-slate-500 mb-1 text-center">{t('profile.preferences.maximum')}</Text>
                    <TextInput
                      className="text-slate-900 text-lg font-bold text-center bg-white rounded-xl px-4 py-3 border border-slate-200"
                      value={String(preferences.max_age || '')}
                      onChangeText={(v) => updateField('preferences', 'max_age', v)}
                      placeholder="35"
                      keyboardType="numeric"
                      placeholderTextColor="#94a3b8"
                    />
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-3">{t('profile.preferences.heightRange')}</Text>
                <View className="flex-row gap-3">
                  <View className="flex-1">
                    <TextInput
                      className="text-slate-900 text-lg font-bold text-center bg-white rounded-xl px-4 py-3 border border-slate-200"
                      value={String(preferences.min_height || '')}
                      onChangeText={(v) => updateField('preferences', 'min_height', v)}
                      placeholder="150"
                      keyboardType="numeric"
                      placeholderTextColor="#94a3b8"
                    />
                  ...
                  <View className="flex-1">
                    <TextInput
                      ...
                      onChangeText={(v) => updateField('preferences', 'max_height', v)}
                      placeholder="180"
                      keyboardType="numeric"
                      placeholderTextColor="#94a3b8"
                    />
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-2">{t('profile.preferences.preferredReligion')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-2">{t('profile.preferences.preferredLocation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={preferences.preferred_country || ''}
                  onChangeText={(v) => updateField('preferences', 'preferred_country', v)}
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-2">{t('profile.preferences.preferredEducation')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
              ...
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-2">{t('profile.preferences.preferredProfession')}</Text>
                <TextInput
                  className="text-slate-900 text-lg font-medium bg-white rounded-xl px-4 py-3 border border-slate-200"
                  value={preferences.profession || ''}
                  onChangeText={(v) => updateField('preferences', 'profession', v)}
          ...
          {activeTab === 'media' && (
            ...
            >
              <View className="flex-row items-center mb-4">
                ...
                <Text className="text-xl font-bold text-slate-900">{t('profile.media.title')}</Text>
              </View>

              {/* Photo Gallery Section */}
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-pink-600 uppercase tracking-wider mb-3">{t('profile.media.photoGallery')}</Text>
                <View className="flex-row flex-wrap gap-2">
                  ...
                  <TouchableOpacity
                    onPress={async () => {
                      ...
                      if (!result.canceled) {
                        Alert.alert(t('profile.alerts.successTitle'), t('profile.alerts.uploadPhotos'));
                      }
                    }}
                  ...
                  >
                    ...
                    <Text className="text-slate-400 text-xs mt-1">{t('profile.media.addPhoto')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              {/* Voice Introduction */}
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-pink-600 uppercase tracking-wider mb-3">{t('profile.media.voiceIntro')}</Text>
                {media.voice_intro ? (
                  <View className="flex-row items-center bg-white rounded-xl p-4 border border-slate-200">
                    ...
                    <View className="flex-1 ml-4">
                      <Text className="text-slate-900 font-semibold">{t('profile.media.voiceIntroLabel')}</Text>
                      <Text className="text-slate-500 text-sm">{t('profile.media.tapToPlay', { duration: media.voice_duration || '0:30' })}</Text>
                    </View>
                  ...
                  <TouchableOpacity className="flex-row items-center bg-white rounded-xl p-4 border-2 border-dashed border-slate-300">
                    ...
                    <View className="flex-1 ml-4">
                      <Text className="text-slate-900 font-semibold">{t('profile.media.recordVoiceIntro')}</Text>
                      <Text className="text-slate-500 text-sm">{t('profile.media.voiceIntroDesc')}</Text>
                    </View>
              ...
              {/* Video Introduction */}
              <View className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
                <Text className="text-xs font-bold text-pink-600 uppercase tracking-wider mb-3">{t('profile.media.videoIntro')}</Text>
                {media.video_intro ? (
                  <View className="relative rounded-xl overflow-hidden">
                    <Image source={{ uri: media.video_thumbnail }} className="w-full h-48" />
                  ...
                  <TouchableOpacity className="flex-row items-center bg-white rounded-xl p-4 border-2 border-dashed border-slate-300">
                    ...
                    <View className="flex-1 ml-4">
                      <Text className="text-slate-900 font-semibold">{t('profile.media.uploadVideoIntro')}</Text>
                      <Text className="text-slate-500 text-sm">{t('profile.media.videoIntroDesc')}</Text>
                    </View>
                    <UploadIcon size={20} color="#94a3b8" />
                  </TouchableOpacity>
                )}
            ...
            >
              {saving ? (
                ...
                <>
                  ...
                  <Text className="text-white font-bold text-lg ml-2">{t('profile.saveChanges')}</Text>
                </>
              )}
            </LinearGradient>
          </TouchableOpacity>
