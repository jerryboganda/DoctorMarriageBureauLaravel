...
import { AlertCircle, Camera, CheckCircle2, Image as ImageIcon, Loader2, Lock, X, Clock } from 'lucide-react';
...
const resolveActionLabel = (snapshot: MediaAccessSnapshot, t: ReturnType<typeof useTranslation>['t'], kind: 'photo' | 'gallery') => {
  if (!snapshot.exists) return t('discovery.notAvailable');
  if (snapshot.accessible) return t('discovery.alreadyAccessible');
  if (snapshot.state === 'approved') return kind === 'photo' ? t('discovery.photoAccessGranted') : t('discovery.galleryAccessGranted');
  if (snapshot.state === 'pending') return kind === 'photo' ? t('discovery.photoAccessRequested') : t('discovery.galleryAccessRequested');
  if (!snapshot.required) return t('discovery.noRequestNeeded');
  return kind === 'photo' ? t('discovery.requestPhotoAccess') : t('discovery.requestGalleryAccess');
};
...
const MediaAccessCard: React.FC<{
  ...
  const actionable = snapshot.exists && snapshot.required && snapshot.state === 'none' && !snapshot.accessible;
  ...
  return (
    <div className={`rounded-2xl border p-4 shadow-sm ${featured ? 'border-primary/25 bg-primary/5 ring-1 ring-primary/10' : 'border-slate-200 bg-slate-50/80'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${statusTone(snapshot)}`}>
      ...
      <div className="mt-3 rounded-xl bg-white border border-slate-200 p-3">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">State</p>
        ...
        >
          {loading ? <Loader2 size={16} className="animate-spin" /> : snapshot.state === 'pending' ? <Clock size={16} /> : snapshot.accessible ? <CheckCircle2 size={16} /> : snapshot.exists ? <Lock size={16} /> : <AlertCircle size={16} />}
...
const MediaAccessRequestModal: React.FC<MediaAccessRequestModalProps> = ({
  ...
  const cardOrder = priorityKind === 'gallery' ? ['gallery', 'photo'] as const : ['photo', 'gallery'] as const;
  const cardConfig = {
    photo: {
      ...
      >
        <div className="px-4 md:px-5 pt-4 pb-3 border-b border-slate-100 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-base md:text-lg font-black text-slate-900">{t('discovery.mediaAccessTitle')}</h3>
          <p className="text-xs md:text-sm text-slate-500 mt-1">
              {t('discovery.mediaAccessSubtitle', { name: profile.name, defaultValue: `Request access for ${profile.name}` })}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 md:p-5 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {cardOrder.map((kind) => (
              <div key={kind} className={kind === priorityKind ? 'md:order-first' : ''}>
                <MediaAccessCard
