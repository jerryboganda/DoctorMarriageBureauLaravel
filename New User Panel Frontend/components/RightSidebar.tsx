  ...
  return createPortal(
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div className={`flex items-center justify-between p-5 border-b border-slate-100 ${accentColor}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
                ...
                <p className="text-sm text-slate-500 font-medium">No profiles to show yet</p>
                <p className="text-xs text-slate-400">Check back later as your profile gets more visibility</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="w-11 h-11 rounded-full overflow-hidden bg-slate-100 shrink-0 ring-2 ring-white shadow-sm">
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(item.name || 'U') + '&background=d41173&color=fff';
                        }}
                      />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-800 truncate">{item.name || 'Unknown'}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        {item.age && (
                          <span className="text-xs text-slate-500">{item.age} yrs</span>
                        )}
                        ...
                        {item.religion && !item.country && (
                          <span className="text-xs text-slate-500">{item.religion}</span>
                        )}
                      </div>
                    </div>
...
const RightSidebar: React.FC<RightSidebarProps> = ({ onNavigateToProfile }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const allSections = [
          ...
          { key: 'preferences', label: t('dashboard.sidebar.preferences', 'Preferences') },
        ...
        const metrics = allSections.map((item) => {
          ...
          const percentage = Math.max(0, Math.min(100, Math.round((value / weight) * 100)));
          return {
            key: item.key,
            label: item.label,
            percentage,
            color: percentage >= 75 ? 'primary' as const : percentage >= 40 ? 'yellow' as const : 'red' as const,
          };
        });
  ...
  const openViewersPopup = useCallback(async () => {
    ...
    try {
      ...
      const items: ProfileItem[] = (Array.isArray(data) ? data : []).filter(Boolean).map((v: any) => ({
        user_id: v.user_id,
        name: v.name || 'Unknown',
        photo: v.photo || '',
  ...
  const levelColor = qualityLevel === 'EXCELLENT' ? 'text-green-600 bg-green-50' :
    qualityLevel === 'GOOD' ? 'text-blue-600 bg-blue-50' :
    qualityLevel === 'FAIR' ? 'text-amber-600 bg-amber-50' :
  ...
  return (
    <aside className="w-[340px] h-full flex flex-col bg-white border-l border-slate-200 shrink-0 p-6 overflow-y-auto hidden xl:flex">
      {/* Dynamic Stats View */}
      <div className="mb-8">
        <h3 className="text-lg font-bold text-slate-900 mb-4">{t('dashboard.sidebar.yourDashboard')}</h3>
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          >
            ...
            <div className="text-xl font-bold text-slate-900">{loading ? '...' : stats.total_views}</div>
          ...
          >
            ...
            <div className="text-xl font-bold text-slate-900">{loading ? '...' : stats.total_likes}</div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-slate-900">{t('dashboard.sidebar.vitalsCheck')}</h3>
          {!loading && qualityLevel && (
            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase ${levelColor}`}>
        ...
        <div className="bg-background-light rounded-xl p-5 space-y-3">
          <p className="text-xs font-medium text-slate-500 mb-2">{t('dashboard.sidebar.basedOnQuality')}</p>
            ...
            <>
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-medium text-slate-700 flex items-center gap-1.5">
                      <span className="text-sm">{SECTION_ICONS[metric.key]}</span>
                      {metric.label}
                    ...
                    <span className={`text-xs font-bold ${
                      ...
                      metric.percentage >= 40 ? 'text-amber-600' :
                      'text-red-500'
                  ...
                  <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${
                        metric.color === 'primary' ? 'bg-green-500' :
                        metric.color === 'yellow' ? 'bg-amber-400' :
                        'bg-red-400'
                      }`}
              ...
              {improvements.length > 0 && (
                <div className="pt-3 border-t border-slate-200 mt-3 space-y-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Quick Improvements</p>
                  {improvements.map((imp, idx) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] text-slate-600 leading-snug group-hover:text-slate-800">{imp.action}</p>
                        <p className="text-[10px] text-primary font-medium mt-0.5">+{imp.points} pts</p>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      {/* Recent Activity */}
      <div>
        <h3 className="text-lg font-bold text-slate-900 mb-4">{t('dashboard.sidebar.recentActivity')}</h3>
        <div className="relative pl-2 border-l-2 border-slate-100 space-y-6">
          {loading ? (
            ...
            <div className="text-sm text-slate-400 text-center py-6">{t('dashboard.sidebar.noRecentActivity')}</div>
          ) : (
              ...
              <div key={activity.id} className="relative pl-6">
                <div
                  ...
                  ${activity.type === 'view' ? 'border-primary' :
                      activity.type === 'message' ? 'border-blue-500' : 'border-emerald-500'}
                `}
                ></div>
      ...
      <div className="mt-auto pt-8">
        <div className="bg-gradient-to-br from-primary/10 to-transparent p-4 rounded-xl border border-primary/20">
          <div className="flex gap-3 mb-2">
            ...
            <h4 className="font-bold text-primary text-sm">{t('dashboard.sidebar.profileTip')}</h4>
          </div>
          <p className="text-xs text-slate-600">
            {t('dashboard.sidebar.tipContent')}
          </p>
        </div>
      </div>
      ...
      {popupType === 'viewers' && (
        <StatsPopup
          ...
          icon={<div className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Eye size={18} /></div>}
          items={popupItems}
      ...
      {popupType === 'interests' && (
        <StatsPopup
          ...
          icon={<div className="p-2 bg-primary/10 text-primary rounded-lg"><Heart size={18} fill="currentColor" /></div>}
          items={popupItems}
          loading={popupLoading}
          onClose={closePopup}
          accentColor="bg-white"
