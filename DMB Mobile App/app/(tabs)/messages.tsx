...
import { View, Text, TextInput, TouchableOpacity, Image, FlatList, KeyboardAvoidingView, Platform, ActivityIndicator } from 'react-native';
...
import { SearchIcon, SendIcon, ChevronRightIcon, MessageIcon, ChevronLeftIcon, PhoneIcon, VideoIcon } from '../../components/Icons';
...
export default function MessagesTab() {
  ...
  const renderThreadItem = ({ item }: { item: ChatThread }) => (
    ...
    >
      {item.opponent_photo ? (
        <Image source={{ uri: item.opponent_photo }} className="w-12 h-12 rounded-full mr-3" />
      ) : (
      ...
      <View className="flex-1">
        <View className="flex-row justify-between items-center mb-1">
          <Text className="font-bold text-slate-900 text-sm" numberOfLines={1}>{item.opponent_name}</Text>
          <Text className="text-slate-400 text-[10px]">{item.last_message_time}</Text>
        </View>
        <Text className="text-slate-500 text-xs" numberOfLines={1}>{item.last_message}</Text>
      </View>
      {item.unseen_message_count > 0 && (
        <View className="ml-2 bg-blue-600 rounded-full px-2 py-0.5 min-w-[20px] items-center">
          <Text className="text-white text-[10px] font-bold">{item.unseen_message_count}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => {
    ...
    return (
      ...
      >
        <View className={`px-4 py-3 rounded-2xl ${isMe ? 'bg-blue-600 rounded-br-md' : 'bg-slate-100 rounded-bl-md'}`}>
          <Text className={`text-sm ${isMe ? 'text-white' : 'text-slate-900'}`}>{item.message}</Text>
        </View>
    ...
    return (
      <View className="flex-1 bg-white">
        ...
        >
          <View className="flex-row items-center px-4 py-3">
            ...
            {selectedThread.opponent_photo ? (
              <Image source={{ uri: selectedThread.opponent_photo }} className="w-11 h-11 rounded-full mr-3" />
            ) : (
              <View className="w-11 h-11 rounded-full bg-blue-100 items-center justify-center mr-3">
                <Text className="text-blue-600 font-bold text-lg">{(selectedThread.opponent_name || 'M')[0]}</Text>
              </View>
            )}
            
            <View className="flex-1">
              <Text className="font-bold text-slate-900 text-base">{selectedThread.opponent_name}</Text>
              <MotiView
                from={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ loop: true, type: 'timing', duration: 1500, repeatReverse: true }}
              >
                <Text className="text-green-500 text-xs font-medium">{t('messages.activeNow')}</Text>
              </MotiView>
            </View>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View className="flex-1 items-center justify-center py-20">
                <Text className="text-slate-400">{t('messages.noMessagesYet')}</Text>
              </View>
            }
          />
        )}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View className="flex-row items-center px-4 py-3 border-t border-slate-100 bg-white" style={{ paddingBottom: insets.bottom + 8 }}>
            <TextInput
            ...
            >
              {sending ? (
                ...
                <SendIcon size={18} color={inputText.trim() ? 'white' : '#94a3b8'} />
              )}
            </TouchableOpacity>
          </View>
  ...
  return (
    <View className="flex-1 bg-slate-50">
      ...
      {/* Header */}
      <View className="px-4 pb-3 bg-white/80 border-b border-slate-100" style={{ paddingTop: insets.top + 12 }}>
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-lg font-bold text-slate-900 text-center mb-2">{t('messages.noConversationsTitle')}</Text>
          <Text className="text-slate-500 text-center text-sm">{t('messages.noConversationsDesc')}</Text>
        </View>
      ) : (
        <FlatList
          data={threads}
