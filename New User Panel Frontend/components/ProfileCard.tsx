...
const ProfileCard: React.FC<ProfileCardProps> = ({ profile, interestId, onDecline, onAccept }) => {
    ...
    const coverGradient =
        displayProfile.coverGradient || 'bg-gradient-to-r from-slate-100 to-slate-200';
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg shadow-slate-200/50 overflow-hidden border border-white relative">
            <div className={`h-40 md:h-56 ${coverGradient} relative`}>
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur text-primary px-3 py-1.5 rounded-full text-xs font-bold shadow-sm flex items-center gap-1">
                ...
                <div className="absolute top-4 left-4">
                    ...
                    {showMenu && (
                        <div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-200">
                            ...
                            >
                                <ShieldAlert size={16} />{' '}
                                {blocking ? t('profile.blocking') : t('profile.blockUser')}
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="pt-16 md:pt-20 flex flex-col md:flex-row justify-between items-start flex-wrap gap-4">
                    <div>
                        ...
                        <p className="text-primary font-medium text-sm md:text-base mt-1">
                            {displayProfile.specialty || t('common.member')}
                        </p>

                        <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-slate-500">
                            <div className="flex items-center gap-1">
                                <MapPin size={16} />
                                {[displayProfile.hospital, displayProfile.location]
                                    .filter(Boolean)
                                    .join(', ') || t('profile.locationUnavailable')}
                            </div>
                            <div className="flex items-center gap-1">
                                <Cake size={16} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                            {(displayProfile.tags && displayProfile.tags.length > 0
                                ? displayProfile.tags
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-bold uppercase tracking-wide text-slate-400">
                            {t('profile.educationAndCareer')}
                        </h3>
                        <ul className="space-y-3">
                            {/* Education entries */}
                            {(displayProfile as any).educations &&
                            (displayProfile as any).educations.length > 0 ? (
                                    ...
                                    <li key={`edu-${i}`} className="flex gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900">
                                                {edu.degree || t('profile.educationNotShared')}
                                            </p>
                                            <p className="text-xs text-slate-500">
                                                {edu.institution ||
                                                    t('profile.institutionUnavailable')}
                                                {edu.start && edu.end
                                                    ? ` (${edu.start} - ${edu.end})`
                                                    : edu.start
                                                      ? ` (${edu.start} - ${t('common.present')})`
                                                      : ''}
                                            </p>
                                        </div>
                                    </li>
                                ...
                                <li className="flex gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-slate-900">
                                            {displayProfile.education?.degree ||
                                                t('profile.educationNotShared')}
                                        </p>
                                        <p className="text-xs text-slate-500">
                                            {displayProfile.education?.institution ||
                                                t('profile.institutionUnavailable')}
                                        </p>
                                    </div>
                                </li>
                            ...
                            {/* Career entries */}
                            {(displayProfile as any).careers &&
                            (displayProfile as any).careers.length > 0 ? (
                                ...
                                <li className="flex gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-slate-900">
                                            {displayProfile.career?.position ||
                                                t('profile.careerNotShared')}
                                        </p>
                                        <p className="text-xs text-slate-500">
                        ...
                        <div className="mt-4 p-3 bg-slate-50 rounded-lg border border-slate-100 flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <p className="text-xs font-bold text-slate-700">
                                    {t('profile.familyDetailsHidden')}
                                </p>
                                <p className="text-[10px] text-slate-500">
                                    {t('profile.unlockFullProfile')}
                                </p>
                            </div>
                        </div>
                    </div>
