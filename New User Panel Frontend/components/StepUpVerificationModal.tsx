...
const StepUpVerificationModal: React.FC<StepUpVerificationModalProps> = ({ purpose, actionLabel, onVerified, onCancel }) => {
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 text-center border-b border-slate-100 relative">
          ...
          <h2 className="text-xl font-bold text-slate-900">{t('modals.stepUp.securityVerification')}</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          {step === 'password' && (
            <>
              ...
              >
                {submitting ? t('modals.stepUp.verifying') : t('modals.stepUp.continue')}
              </button>
            </>
          )}
          ...
          {step === 'otp' && (
            <>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-2">{t('modals.stepUp.verificationCode')}</label>
                <input
                  type="text"
              ...
              <button
                ...
                className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-slate-800 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {submitting ? t('modals.stepUp.verifying') : t('modals.stepUp.confirm')}
              </button>
              <button onClick={() => setStep('password')} className="w-full text-sm text-slate-500 hover:text-slate-800">
