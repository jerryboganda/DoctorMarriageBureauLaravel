...
const Input = forwardRef<TextInput, InputProps>(({ 
  label, icon, error, className, containerClassName, 
  onFocus, onBlur, value, onChangeText, defaultValue, 
  type, onClick, placeholder,
  ...props 
}, ref) => {
  const [isFocused, setIsFocused] = useState(false);
  
  const secureTextEntry = type === 'password' || props.secureTextEntry;
  const keyboardType = type === 'email' ? 'email-address' : (type === 'tel' ? 'phone-pad' : props.keyboardType);
  ...
  const labelStyle = useAnimatedStyle(() => {
    const translateY = interpolate(animationProgress.value, [0, 1], [0, -18], Extrapolation.CLAMP);
    const scale = interpolate(animationProgress.value, [0, 1], [1, 0.75], Extrapolation.CLAMP);
    const translateX = interpolate(animationProgress.value, [0, 1], [0, icon ? -28 : -8], Extrapolation.CLAMP);
  ...
  return (
    <View className={cn("w-full mb-5", containerClassName)}>
        ...
        {label && (
           <Animated.View 
             style={[labelStyle]} 
             className={cn("absolute top-[18px] z-20 pointer-events-none", icon ? "left-12" : "left-4" )}
           >
              <Text className={cn("text-base font-medium text-slate-400", isFocused ? "text-blue-600" : "")}>
                {label}
              </Text>
           </Animated.View>
        )}
        ...
        >
          ...
          <View className="flex-1 h-full justify-center pt-1">
             <TextInput
               ref={ref}
               className={cn("w-full text-base font-semibold text-slate-900 h-full", className)}
               onFocus={(e) => {
                 setIsFocused(true);
                 onFocus?.(e);
               }}
