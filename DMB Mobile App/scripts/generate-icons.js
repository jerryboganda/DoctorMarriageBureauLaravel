...
function buildLogoSvg(size, { withText = false, padding = 0.08 } = {}) {
    ...
    <text x="${cx}" y="${cy + r * 1.5}" text-anchor="middle" font-family="Arial Black, Helvetica, sans-serif" font-weight="900" font-size="${s * 0.07}" fill="#2563EB" letter-spacing="${s * 0.006}">BUREAU</text>
  ...
  <g transform="translate(${cx - r * 0.08 - ww}, ${wingY})">
    <path d="M${ww} ${wh * 0.48} C ${ww * 0.78} ${wh * 0.15}, ${ww * 0.55} ${wh * 0.0}, ${ww * 0.4} ${wh * 0.15} C ${ww * 0.28} ${wh * 0.28}, ${ww * 0.15} ${wh * 0.05}, 0 ${wh * 0.22} C ${ww * 0.12} ${wh * 0.38}, ${ww * 0.35} ${wh * 0.7}, ${ww * 0.58} ${wh * 0.72} C ${ww * 0.75} ${wh * 0.73}, ${ww * 0.92} ${wh * 0.62}, ${ww} ${wh * 0.48} Z" fill="#EF4444" opacity="0.92"/>
    <path d="M${ww} ${wh * 0.56} C ${ww * 0.8} ${wh * 0.3}, ${ww * 0.55} ${wh * 0.18}, ${ww * 0.38} ${wh * 0.3} C ${ww * 0.25} ${wh * 0.4}, ${ww * 0.1} ${wh * 0.22}, ${ww * 0.02} ${wh * 0.36} C ${ww * 0.15} ${wh * 0.5}, ${ww * 0.4} ${wh * 0.82}, ${ww * 0.62} ${wh * 0.82} C ${ww * 0.78} ${wh * 0.82}, ${ww * 0.93} ${wh * 0.7}, ${ww} ${wh * 0.56} Z" fill="#DC2626" opacity="0.55"/>
  </g>
  ...
  <!-- Left helix -->
  <path d="M${cx} ${cy + r * 0.6} Q ${cx + r * 0.26} ${cy + r * 0.35} ${cx} ${cy + r * 0.15} Q ${cx - r * 0.26} ${cy - r * 0.05} ${cx} ${cy - r * 0.22} Q ${cx + r * 0.18} ${cy - r * 0.35} ${cx} ${cy - r * 0.42}" fill="none" stroke="#2563EB" stroke-width="${s * 0.01}" stroke-linecap="round"/>
  <!-- Right helix -->
  <path d="M${cx} ${cy + r * 0.6} Q ${cx - r * 0.26} ${cy + r * 0.35} ${cx} ${cy + r * 0.15} Q ${cx + r * 0.26} ${cy - r * 0.05} ${cx} ${cy - r * 0.22} Q ${cx - r * 0.18} ${cy - r * 0.35} ${cx} ${cy - r * 0.42}" fill="none" stroke="#2563EB" stroke-width="${s * 0.01}" stroke-linecap="round"/>

  <!-- Halo -->
  <ellipse cx="${cx}" cy="${cy - r * 0.72}" rx="${r * 0.2}" ry="${r * 0.09}" fill="none" stroke="#9CA3AF" stroke-width="${s * 0.007}" opacity="0.7"/>
  <ellipse cx="${cx}" cy="${cy - r * 0.72}" rx="${r * 0.2}" ry="${r * 0.09}" fill="none" stroke="#D1D5DB" stroke-width="${s * 0.003}" opacity="0.3"/>
  ${textSection}
</svg>`;
}
...
async function main() {
    ...
    for (const [folder, size] of Object.entries(mipmaps)) {
        ...
        const iconBuf = await sharp(Buffer.from(buildIconSvg(512)))
            .resize(size, size)
            .png()
            .toBuffer();
        ...
        const fgBuf = await sharp(Buffer.from(buildAdaptiveFgSvg(512)))
            .resize(size, size)
            .png()
            .toBuffer();
        await sharp(fgBuf)
    ...
    await sharp(Buffer.from(logoWithTextSvg))
        .resize(800, 800)
        .png()
