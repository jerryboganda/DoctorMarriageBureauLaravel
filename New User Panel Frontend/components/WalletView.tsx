...
const WalletView: React.FC = () => {
    ...
    const [withdrawMeta, setWithdrawMeta] = useState<PaginationMeta>({
        current_page: 1,
        last_page: 1,
    });
    ...
    const selectedMethod = useMemo(() => {
        ...
        return (
            paymentMethods.find((method) => method.payment_type_key === selectedMethodKey) || null
        );
    ...
    const fetchPaymentMethods = useCallback(async () => {
        const response = await api.get('/payment-types', {
            params: { payment_type: 'wallet_recharge' },
        });
    ...
    const fetchWithdraws = useCallback(async (page = 1) => {
        try {
            const response = await api.get('/member/wallet-withdraw-request-history', {
                params: { page },
            });
        ...
        try {
            ...
            if (selectedMethod.payment_type_key === 'paypal') {
                ...
                if (response.data?.url) {
                    ...
                    setGatewayNotice(
                        'A payment window has opened in a new tab. Complete the top-up there, then refresh your wallet balance.',
                    );
                    return;
                }
            ...
            window.open(
                `${api.defaults.baseURL}/${path}?${params.toString()}`,
                '_blank',
                'noopener',
            );
            setGatewayNotice(
            ...
            <header className="h-auto md:h-20 shrink-0 flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-8 py-4 bg-white border-b border-slate-200 sticky top-0 z-10 gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Manage your balance, top up funds, and track wallet activity.
                    </p>
                </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                <div className="lg:col-span-2 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl p-6 md:p-8 shadow-xl">
                                    <div className="flex items-start justify-between gap-4">
                                        <div>
                                            ...
                                            <p className="text-white/70 text-sm mt-3 max-w-xl">
                                                Use your wallet balance for package and add-on
                                                purchases directly from checkout.
                                            </p>
                                        </div>
                                    ...
                                    <div className="flex flex-wrap gap-3 mt-8">
                                        <button
                                            onClick={() => setShowRechargeModal(true)}
                                            className="px-4 py-2.5 bg-white text-slate-900 rounded-xl text-sm font-bold inline-flex items-center gap-2"
                                        >
                                            <CreditCard size={16} /> Add Funds
                                        </button>
                                        <button
                                            onClick={() => setTransactionPage(1)}
                                            className="px-4 py-2.5 bg-white/10 text-white rounded-xl text-sm font-semibold inline-flex items-center gap-2 border border-white/10"
                                        >
                                ...
                                <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                                    ...
                                    <ul className="space-y-3 text-sm text-slate-600">
                                        <li className="flex gap-3">
                                            ...
                                            />
                                            <span>
                                                Top up your wallet before choosing Wallet Payment in
                                                checkout.
                                            </span>
                                        </li>
                                        <li className="flex gap-3">
                                            ...
                                            />
                                            <span>
                                                Manual top-ups stay pending until approved by the
                                                team.
                                            </span>
                                        </li>
                                        <li className="flex gap-3">
                                            ...
                                            />
                                            <span>
                                                Use Refresh after completing an external gateway
                                                payment.
                                            </span>
                                        </li>
                                    </ul>
                                ...
                                <section className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                                    <div className="flex items-center justify-between gap-4 mb-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 mt-1">
                                                All wallet credits and recharge requests.
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <section className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                                    <div className="flex items-center justify-between gap-4 mb-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 mt-1">
                                                {withdrawEnabled
                                                    ? 'Request a payout from eligible wallet balance.'
                                    ...
                                    {withdrawEnabled ? (
                                        ...
                                        <div className="rounded-2xl border border-dashed border-slate-200 px-6 py-10 text-center text-sm text-slate-500">
                                            Withdraw requests are currently unavailable for your
                                            account.
                                        </div>
                                    )}
                                </section>
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
                        <div className="flex items-center justify-between p-6 border-b border-slate-100">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-1">
                                    Choose a payment method and add funds to your wallet.
                                </p>
                            </div>
                            <button
                                onClick={resetRechargeModal}
                        ...
                        <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    {paymentMethods.map((method) => {
                                        const selected =
                                            selectedMethodKey === method.payment_type_key;
                                        return (
                                            <button
                                                key={method.payment_type_key}
                                                onClick={() =>
                                                    setSelectedMethodKey(method.payment_type_key)
                                                }
                                            ...
                                            >
                                                <div>
                                                    <p
                                                        className={`font-bold ${selected ? 'text-primary' : 'text-slate-800'}`}
                                                    >
                            ...
                            {selectedMethod?.payment_type === 'manual_payment' ? (
                                <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                    {selectedMethod.details ? (
                                        <div
                                            className="text-xs text-slate-600 leading-6 bg-white border border-slate-200 rounded-xl p-3"
                                            dangerouslySetInnerHTML={{
                                                __html: selectedMethod.details,
                                            }}
                                        />
                                    ) : null}
                                    <input
                            ...
                            {rechargeError ? (
                                <p className="text-sm text-red-600">{rechargeError}</p>
                            ) : null}
                            {gatewayNotice ? (
                                <p className="text-sm text-green-600">{gatewayNotice}</p>
                            ) : null}

                            <div className="flex flex-col-reverse sm:flex-row gap-3 pt-2">
                                <button
                                    onClick={resetRechargeModal}
                                    className="flex-1 px-4 py-3 rounded-xl border border-slate-200 text-slate-700 font-semibold text-sm"
                                >
                                    {t('common.cancel')}
                                </button>
                                <button
