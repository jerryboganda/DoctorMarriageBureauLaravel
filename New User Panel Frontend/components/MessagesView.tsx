...
const isSubscriptionRequiredError = (error: any): boolean => {
    return (
        error?.response?.status === 403 && error?.response?.data?.code === 'SUBSCRIPTION_REQUIRED'
    );
};

const isVerificationRequiredError = (error: any): boolean => {
    return (
        error?.response?.status === 403 && error?.response?.data?.code === 'VERIFICATION_REQUIRED'
    );
};
...
const MessagesView: React.FC<MessagesViewProps> = ({
                ...
                thread.id === incoming.chat_thread_id
                    ? {
                          ...
                          unseen_message_count:
                              incoming.sender_user_id === currentUserId
    ...
    useEffect(() => {
        ...
        const targetThread = threads.find(
            (thread) => String(thread.user_id) === String(initialMemberId),
        );
    ...
    const doSendMessage = async (text: string) => {
        ...
        const optimisticMsg = {
            id: `tmp-${selectedChatId}-${++optimisticMessageSeq.current}`,
    ...
    const handleScheduleCall = () => {
        setShowScheduleModal(false);
        doSendMessage(
            "📞 I'd like to schedule a call with you. When would be a good time to talk? Please suggest a day and time that works for you.",
        );
    };
    ...
    return (
        <div className="flex h-full bg-white relative">
            ...
            <div
                className={`w-full md:w-80 lg:w-96 flex-col border-r border-slate-200 bg-white ${selectedChatId ? 'hidden md:flex' : 'flex'}`}
            >
                {/* Header */}
                                ...
                                >
                                    ...
                                    <div className="flex-1 overflow-hidden min-w-0">
                                        <div className="flex justify-between items-baseline mb-0.5">
                                            <h4
                                                className={`truncate text-sm ${hasUnread ? 'font-bold text-slate-900' : 'font-medium text-slate-800'}`}
                                            >
                                                {thread.member_name}
                                            </h4>
                                            <span
                                                className={`text-[11px] ml-2 shrink-0 ${hasUnread ? 'text-primary font-semibold' : 'text-slate-400'}`}
                                            >
                                        ...
                                        <div className="flex items-center gap-2">
                                            <p
                                                className={`text-xs truncate flex-1 ${hasUnread ? 'font-medium text-slate-700' : 'text-slate-400'}`}
                                            >
                                                {thread.last_message ||
            ...
            <div
                className={`flex-1 flex-col h-full ${!selectedChatId ? 'hidden md:flex' : 'flex'}`}
            >
                {selectedThread ? (
                    <>
                        {/* Chat Header */}
                            ...
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="relative shrink-0">
                                    <div className="size-10 rounded-full bg-slate-200 overflow-hidden">
                                        <img
                                            src={selectedThread.member_photo}
                                            className="w-full h-full object-cover"
                                            alt=""
                                        />
                                    </div>
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p
                                        className={`text-[11px] leading-tight ${isUserOnline(selectedThread) ? 'text-green-600' : 'text-slate-400'}`}
                                    >
                                        {isUserOnline(selectedThread)
                                            ? t('messages.online')
                                            : t('messages.offline')}
                                    ...
                                    {messageGroups.map((group, gi) => (
                                        <div key={gi}>
                                            ...
                                            {group.messages.map((msg: any, mi: number) => {
                                                ...
                                                const isConsecutive =
                                                    mi > 0 &&
                                                    group.messages[mi - 1].sender_user_id ===
                                                        msg.sender_user_id;
                                                                                ...
                                                                                (
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <div className="flex-1 min-w-0">
                                                                                            <p
                                                                                                className={`text-xs font-medium truncate ${isMine ? 'text-white' : 'text-slate-700'}`}
                                                                                            >
                                                                                                {att.file_name ||
                                                                                                    'Document'}
                                                                                                .
                                                                                            ...
                                                                                            <p
                                                                                                className={`text-[10px] ${isMine ? 'text-white/60' : 'text-slate-400'}`}
                                                                                            >
                                ...
                                <div className="absolute bottom-full left-3 md:left-5 mb-1 w-[340px] bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
                                    <div className="px-3 py-2.5 border-b border-slate-100 flex items-center justify-between">
                                        <p className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                                            ...
                                            {t('messages.chooseIcebreaker')}
                                        </p>
                                        <button
                                            onClick={() => setShowIcebreakerMenu(false)}
