...
import { View, Text, TouchableOpacity, TextInput, Dimensions, Modal, StyleSheet, Platform } from 'react-native';
import { MotiView } from 'moti';
import { FlashList } from '@shopify/flash-list';
import { Image } from 'expo-image';
import { Canvas, Rect, SweepGradient, vec, LinearGradient as SkiaLinearGradient } from '@shopify/react-native-skia';
...
// Icons
import { 
    FilterIcon, SearchIcon, SettingsIcon, CrownIcon, MessageIcon, 
    StethoscopeIcon, ChevronRightIcon, UserIcon, ShieldIcon
} from './Icons';
...
// --- Mock Data ---
const MOCK_PROFILES = [
    { id: 1, name: 'Dr. Sarah', age: 29, specialty: 'Cardiologist', education: 'MD, Harvard', location: 'New York, NY', matchScore: 98, avatarUrl: 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?q=80&w=800&auto=format&fit=crop' },
    { id: 2, name: 'Dr. James', age: 32, specialty: 'Neurologist', education: 'MD, Hopkins', location: 'Chicago, IL', matchScore: 85, avatarUrl: 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?q=80&w=800&auto=format&fit=crop' },
    { id: 3, name: 'Dr. Emily', age: 27, specialty: 'Pediatrician', education: 'MD, Stanford', location: 'San Francisco, CA', matchScore: 92, avatarUrl: 'https://images.unsplash.com/photo-1594824476967-48c8b964273f?q=80&w=800&auto=format&fit=crop' },
    { id: 4, name: 'Dr. Michael', age: 34, specialty: 'Surgeon', education: 'MD, Yale', location: 'Boston, MA', matchScore: 95, avatarUrl: 'https://images.unsplash.com/photo-1622253692010-333f2da6031d?q=80&w=800&auto=format&fit=crop' },
];

const RECENT_MATCHES_GRID = [
    { id: 201, name: 'Dr. Priya', specialty: 'Dermatologist', age: 28, score: 96, location: 'Mumbai', verified: true },
    { id: 202, name: 'Dr. Amit', specialty: 'Neurosurgeon', age: 31, score: 89, location: 'Delhi', verified: true },
    { id: 203, name: 'Dr. Neha', specialty: 'Pediatrician', age: 26, score: 92, location: 'Bangalore', verified: false },
    { id: 204, name: 'Dr. Arjun', specialty: 'Cardiologist', age: 33, score: 85, location: 'Hyderabad', verified: true },
];

const CHAT_LIST = [
    { id: 1, name: 'Dr. Ayesha', message: 'That sounds wonderful! When are you free?', time: '2m', unread: 1, avatar: 'https://images.unsplash.com/photo-1527613426441-4da17471b66d?w=200' },
    { id: 2, name: 'Dr. Raj', message: 'I actually specialize in pediatric surgery too.', time: '1h', unread: 0, avatar: 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=200' },
];
...
export default function Dashboard() {
    ...
    const Header = () => (
        <View className="px-5 pt-14 pb-4 flex-row justify-between items-center z-10 bg-white/50 backdrop-blur-md sticky top-0">
            ...
            <View className="flex-row gap-3">
                ...
                <TouchableOpacity 
                    onPress={() => activeTab === 'discover' ? toggleFilter('filter') : activeTab === 'profile' ? setSettingsView('account') : null}
                    className="w-10 h-10 rounded-full bg-white border border-slate-200 items-center justify-center shadow-sm"
                >
                     {activeTab === 'profile' ? <SettingsIcon size={20} color="#475569" /> : 
        ...
        <FlashList
            ...
            ListHeaderComponent={() => (
                <View className="px-6 mb-4 flex-row justify-between items-center">
                    <Text className="text-slate-500 font-semibold text-xs uppercase tracking-widest">Daily Recommendations</Text>
    ...
    const renderMatchItem = ({ item }: { item: any }) => (
        ...
        >
            <View className="h-28 bg-indigo-50 rounded-2xl mb-3 overflow-hidden relative">
                 <Image source={{ uri: `https://i.pravatar.cc/150?u=${item.id}` }} style={{ width: '100%', height: '100%' }} contentFit="cover" />
            ...
            <TouchableOpacity 
                onPress={() => setSelectedChat({ id: item.id, name: item.name, message: 'New Match' })}
                className="mt-3 bg-slate-900 py-2.5 rounded-xl flex-row justify-center items-center gap-2"
            >
                 <MessageIcon size={12} color="white" />
                 <Text className="text-white text-[10px] font-bold uppercase tracking-wide">Connect</Text>
            </TouchableOpacity>
        </TouchableOpacity>
    );
    ...
    const renderChatItem = ({ item }: { item: any }) => (
        ...
        >
            ...
            <View className="ml-4 flex-1">
                ...
                <Text className="text-slate-600 text-sm font-medium" numberOfLines={1}>{item.message}</Text>
            </View>
            {item.unread > 0 && (
            ...
            renderItem={() => (
                <View className="px-5 pt-4 pb-32">
                     <View className="items-center mb-8">
                         ...
                         <View className="flex-row items-center gap-2 mt-2">
                             <View className="bg-blue-50 px-3 py-1.5 rounded-lg flex-row items-center gap-1.5 border border-blue-100">
                                 ...
                                 <Text className="text-blue-700 text-xs font-bold uppercase">Cardiologist</Text>
                             </View>
                         </View>
                     </View>
                     ...
                     <View className="flex-row justify-between mb-6 bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
                         <View>
                             <Text className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Views</Text>
                             <Text className="text-3xl font-black text-slate-900">1.2k</Text>
                         </View>
                         <View className="h-full w-[1px] bg-slate-100" />
                         <View className="items-center">
                             <Text className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Matches</Text>
                             <Text className="text-3xl font-black text-slate-900">48</Text>
                         </View>
                         <View className="h-full w-[1px] bg-slate-100" />
                         <View className="items-end">
                             <Text className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Rating</Text>
                             <Text className="text-3xl font-black text-slate-900">4.9</Text>
                         </View>
                     </View>

                     <TouchableOpacity onPress={() => setSettingsView('account')} className="bg-white p-5 rounded-3xl mb-4 border border-slate-100 flex-row items-center justify-between shadow-sm active:bg-slate-50">
                         <View className="flex-row items-center gap-4">
                             ...
                             <View>
                                 ...
                                 <Text className="text-xs text-slate-500 font-medium">Manage details & photos</Text>
                             </View>
                         </View>
                         <ChevronRightIcon size={20} color="#cbd5e1" />
                     </TouchableOpacity>

                     <TouchableOpacity onPress={() => setSettingsView('privacy')} className="bg-white p-5 rounded-3xl mb-4 border border-slate-100 flex-row items-center justify-between shadow-sm active:bg-slate-50">
                         <View className="flex-row items-center gap-4">
                             ...
                             <View>
                                 ...
                                 <Text className="text-xs text-slate-500 font-medium">Visibility & Security</Text>
                             </View>
                         </View>
                         <ChevronRightIcon size={20} color="#cbd5e1" />
                     </TouchableOpacity>
    ...
    return (
        <View className="flex-1 bg-slate-50">
            ...
            <Modal visible={!!selectedProfile} animationType="fade" transparent onRequestClose={() => setSelectedProfile(null)}>
            ...
            <Modal visible={!!selectedChat} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setSelectedChat(null)}>
                {selectedChat && (
                     <ChatDetail 
                        chat={selectedChat} 
            ...
            <Modal visible={!!settingsView} animationType="slide" onRequestClose={() => setSettingsView(null)}>
                <SettingsScreen 
