...
import { XIcon, CheckIcon, FilterIcon } from './Icons';
...
const SPECIALTIES = ['Cardiology', 'Neurology', 'Surgery', 'Pediatrics', 'Dermatology', 'Psychiatry'];
        ...
        >
            <View className="flex-1 bg-slate-50">
                ...
                <ScrollView className="flex-1 p-5">
                    {/* Specialty Section */}
                    <View className="mb-8">
                        <Text className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">{t('filter.medicalSpecialty')}</Text>
                        <View className="flex-row flex-wrap gap-3">
                            {SPECIALTIES.map(spec => {
                                ...
                                return (
                                    <TouchableOpacity 
                                        key={spec}
                                        onPress={() => toggleSelection(selectedSpecialties, setSelectedSpecialties, spec)}
                                        className={`px-4 py-2.5 rounded-xl border ${isSelected ? 'bg-blue-600 border-blue-600' : 'bg-white border-slate-200'}`}
                                    >
                                        <Text className={`font-bold text-sm ${isSelected ? 'text-white' : 'text-slate-600'}`}>
                                            {spec}
                                        </Text>
                                    </TouchableOpacity>
                                );
                    ...
                    {/* Religion Section */}
                    <View className="mb-8">
                        <Text className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">{t('filter.religionCommunity')}</Text>
                        <View className="flex-row flex-wrap gap-3">
                            {RELIGIONS.map(rel => {
                                ...
                                return (
                                    <TouchableOpacity 
                                        key={rel}
                                        onPress={() => toggleSelection(selectedReligions, setSelectedReligions, rel)}
                    ...
                    <View className="bg-white p-5 rounded-2xl border border-slate-100 flex-row items-center justify-between mb-8">
                         <View className="flex-1 pr-4">
                             <Text className="font-bold text-slate-900 text-lg mb-1">{t('filter.strictMode')}</Text>
                             <Text className="text-slate-500 text-xs">{t('filter.strictModeDesc')}</Text>
                         </View>
                         <Switch 
                            value={strictMode} 
                ...
                <View className="p-5 bg-white border-t border-slate-100 flex-row gap-4 mb-5">
                    ...
                    >
                        <Text className="font-bold text-white text-lg">{t('filter.showMatches')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </Modal>
