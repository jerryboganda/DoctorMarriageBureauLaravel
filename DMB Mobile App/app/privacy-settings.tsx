...
import { 
  ChevronLeftIcon, ShieldIcon, EyeIcon, EyeOffIcon, 
  UserIcon, ImageIcon, PhoneIcon, MailIcon, MapPinIcon,
  LockIcon, GlobeIcon, UsersIcon, CheckCircleIcon
  ...
  const updateSetting = async (key: keyof PrivacySettings, value: any) => {
    ...
    try {
      ...
      if (!response.data.success) {
        ...
        Alert.alert(t('common.error'), response.data.message || t('privacySettings.updateFailed'));
      }
    } catch (error) {
      setSettings(prev => ({ ...prev, [key]: previousValue }));
  ...
  const saveAllSettings = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        Alert.alert(t('common.error'), response.data.message || t('privacySettings.saveFailed'));
      }
    } catch (error) {
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        <View className="flex-row items-center justify-between px-4 py-4">
          ...
          <Text className="text-xl font-bold text-white">{t('privacySettings.title')}</Text>
      ...
      <ScrollView className="flex-1 px-4 -mt-4" showsVerticalScrollIndicator={false}>
        ...
        >
          <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('privacySettings.profileVisibility')}</Text>
        ...
        <MotiView
          ...
          className="bg-white rounded-2xl p-4 shadow-sm mb-4"
        >
          <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('privacySettings.contactInfo')}</Text>
        ...
        <MotiView
          ...
          className="bg-white rounded-2xl p-4 shadow-sm mb-4"
        >
          <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('privacySettings.activityStatus')}</Text>
        ...
        <MotiView
          ...
          className="bg-white rounded-2xl p-4 shadow-sm mb-4"
        >
          <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('privacySettings.messaging')}</Text>
        ...
        <MotiView
          ...
          className="bg-white rounded-2xl p-4 shadow-sm mb-4"
        >
          <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('privacySettings.security')}</Text>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text className="text-white font-bold text-lg ml-2">{t('privacySettings.saveAll')}</Text>
              </>
            )}
          </LinearGradient>
        </TouchableOpacity>
