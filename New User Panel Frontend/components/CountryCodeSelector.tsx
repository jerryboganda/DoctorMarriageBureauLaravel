    ...
    <>
      ...
      >
        ...
        <div className="p-3 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50 shrink-0">
          ...
          {isMobile && (
            <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 transition-colors shrink-0">
              <X size={20} />
            </button>
          )}
        </div>
            ...
            filteredCountries.map((country) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-slate-900 truncate">{country.name}</p>
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <span className={`${variant === 'simple' ? 'text-xs' : 'text-sm'} whitespace-nowrap`}>{selectedCountry.dialCode}</span>
