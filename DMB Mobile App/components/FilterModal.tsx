...
const PROFESSIONS = ['Doctor', 'Engineer', 'Surgeon', 'Specialist', 'Consultant', 'Professor', 'Researcher'];
              ...
              {(['Male', 'Female', 'Any'] as const).map((gender) => (
                ...
                >
                  <Text className={`font-bold ${
                    filters.gender === gender ? 'text-blue-700' : 'text-slate-600'
                  }`}>
                    {gender}
                  </Text>
                </TouchableOpacity>
            ...
            <View className="bg-white rounded-2xl p-4 border border-slate-100">
              ...
              <View className="flex-row items-center gap-4">
                <View className="flex-1">
                  <Text className="text-xs text-slate-400 mb-2 text-center">{t('filter.min')}</Text>
                  <ScrollView 
                    horizontal 
                    showsHorizontalScrollIndicator={false}
                    className="bg-slate-50 rounded-xl p-2"
                  >
                    {AGE_OPTIONS.filter(a => a <= filters.ageMax).map((age) => (
                      <TouchableOpacity
                        key={`min-${age}`}
                        onPress={() => setFilters({ ...filters, ageMin: age })}
                        className={`w-12 h-10 rounded-lg items-center justify-center mr-1 ${
                          filters.ageMin === age ? 'bg-blue-600' : 'bg-white'
                        }`}
                      >
                        <Text className={`font-semibold ${
                          filters.ageMin === age ? 'text-white' : 'text-slate-600'
                        }`}>
                          {age}
                        </Text>
                      </TouchableOpacity>
                ...
                <View className="flex-1">
                  <Text className="text-xs text-slate-400 mb-2 text-center">{t('filter.max')}</Text>
                  <ScrollView 
                    horizontal 
                    showsHorizontalScrollIndicator={false}
                    className="bg-slate-50 rounded-xl p-2"
                  >
                    {AGE_OPTIONS.filter(a => a >= filters.ageMin).map((age) => (
                      <TouchableOpacity
                        key={`max-${age}`}
                        onPress={() => setFilters({ ...filters, ageMax: age })}
                        className={`w-12 h-10 rounded-lg items-center justify-center mr-1 ${
                          filters.ageMax === age ? 'bg-blue-600' : 'bg-white'
                        }`}
                      >
                        <Text className={`font-semibold ${
                          filters.ageMax === age ? 'text-white' : 'text-slate-600'
                        }`}>
                          {age}
                        </Text>
                      </TouchableOpacity>
