...
class ProfileController extends Controller
{
    ...
    public function public_profile(Request $request, $id)
    {
        ...
        $readOnlyProfileView = $request->boolean('read_only')
        ...
        if ($user) {
            ...
            $canViewContact = $auth_user && (
                (int) $auth_user->id === (int) $user->id
                || ViewContact::where('user_id', $user->id)->where('viewed_by', $auth_user->id)->exists()
            );
            $data['contact_details']['email'] = $canViewContact ? $user->email : null;
            $data['contact_details']['phone'] = $canViewContact ? $user->phone : null;
    ...
    {
        $user = User::where('id', auth()->id())->first();
        if ($request->filled('email') && strcasecmp((string) $request->email, (string) $user->email) !== 0) {
            return response()->json([
                'result' => false,
    ...
    public function get_full_profile_react()
    {
        ...
        // Server truth for gate: an account is onboarded once it is either
        // explicitly marked complete, or its required data is fully present.
        // Never downgrade an already-completed account when the required
        // field list gets stricter later - that would retroactively lock out
        // users who legitimately finished onboarding under the rules at the
        // time, forcing them back through the whole wizard.
        ...
        // Auto-complete accounts whose data is fully populated but the flag
        // was never set (e.g. legacy imports).
        if (! $member->onboarding_completed && $onboardingCompleted) {
        ...
        return response()->json([
            ...
            'basics' => [
                ...
                'avatarUrl' => $user->photo
                    ? ((strpos($user->photo, 'http') === 0)
                        ? $user->photo
                        : (uploaded_asset($user->photo) ?? gender_avatar($member)))
                    : gender_avatar($member),
    ...
    public function update_full_profile_react(Request $request)
    {
        ...
        // A user_type=member account should always have a member row, but a
        // handful of older/edge-case accounts ended up without one. This is
        // exactly the flow meant to populate that data for the first time,
        // so create it on demand rather than dead-ending with a 404.
