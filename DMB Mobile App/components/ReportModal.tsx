...
import { ShieldIcon, ChevronRightIcon, XCircleIcon, CheckCircleIcon } from './Icons';
            ...
            <View className="flex-1 justify-end bg-black/50">
                ...
                >
                    <View className="flex-row justify-between items-center mb-6">
                        <Text className="text-xl font-bold text-slate-900">{t('report.title', { name: userName })}</Text>
                    ...
                    {step === 1 ? (
                                ...
                                {REPORT_REASONS.map((r) => (
                                    <TouchableOpacity
                                        ...
                                        className={`p-4 rounded-xl border flex-row justify-between items-center ${
                                            reason === r.label ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100'
                                        }`}
                                    >
                                        <View>
                                            <Text className={`font-bold ${reason === r.label ? 'text-red-700' : 'text-slate-900'}`}>
                                                {r.label}
                                            </Text>
                                            <Text className="text-xs text-slate-500">{r.desc}</Text>
                                        </View>
                                        {reason === r.label && <CheckCircleIcon size={20} color="#dc2626" />}
                                    </TouchableOpacity>
                                ))}
                            </View>

                            <Text className="font-bold text-slate-900 mb-2">{t('report.additionalDetails')} <Text className="text-[10px] font-semibold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full">Optional</Text></Text>
                            <TextInput
                        ...
                        <View className="flex-1 items-center justify-center -mt-20">
                            ...
                            <Text className="text-2xl font-bold text-slate-900">{t('report.reportReceived')}</Text>
