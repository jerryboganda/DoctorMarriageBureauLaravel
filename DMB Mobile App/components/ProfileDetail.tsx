...
import { View, Text, TouchableOpacity, Image, Dimensions, Alert, Modal, ScrollView } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { 
    ChevronLeftIcon, MapPinIcon, BriefcaseIcon, GraduationCapIcon, 
    HeartIcon, UsersIcon, ShieldIcon, CheckIcon, StarIcon,
    PhoneIcon, FlagIcon, XCircleIcon
} from './Icons';
                    ...
                    <View className="p-6 -mt-6 bg-white rounded-t-[32px]">
                        ...
                        <View className="flex-row justify-between mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <View className="items-center">
                                ...
                                <Text className="text-slate-900 font-bold">{profile.matchPercentage}%</Text>
                                <Text className="text-slate-500 text-xs">{t('profileDetail.match')}</Text>
                            </View>
                            <View className="items-center">
                                <View className="w-10 h-10 bg-green-100 rounded-full items-center justify-center mb-1">
                                    <MapPinIcon size={20} color="#16a34a" />
                                </View>
                                <Text className="text-slate-900 font-bold" numberOfLines={1}>{profile.location?.split(',')[0] || 'Unknown'}</Text>
                                <Text className="text-slate-500 text-xs">{t('profileDetail.location')}</Text>
                            </View>
                            <View className="items-center">
                                ...
                                <Text className="text-slate-900 font-bold" numberOfLines={1}>{profile.degree || 'MBBS'}</Text>
                                <Text className="text-slate-500 text-xs">{t('profileDetail.education')}</Text>
                            </View>
                        </View>

                        <Section title={t('profileDetail.about')} icon={<UsersIcon size={18} color="#64748b" />}>
                            <Text className="text-slate-600 leading-6">
                                {profile.about || t('profileDetail.defaultAbout', { name: profile.name, specialty: profile.specialty, hospital: profile.hospital || 'a leading hospital' })}
                            </Text>
                        </Section>

                        <Section title={t('profileDetail.basicDetails')} icon={<BriefcaseIcon size={18} color="#64748b" />}>
                            <DetailRow label={t('profileDetail.age')} value={t('profileDetail.years', { age: profile.age })} />
                            <DetailRow label={t('profileDetail.height')} value={profile.height || "5'8\""} />
                            <DetailRow label={t('profileDetail.religion')} value={profile.religion || t('profileDetail.notSpecified')} />
                            <DetailRow label={t('profileDetail.community')} value={profile.caste || t('profileDetail.notSpecified')} />
                            <DetailRow label={t('profileDetail.maritalStatus')} value={profile.marital_status || t('profileDetail.neverMarried')} />
                        </Section>

                        <Section title={t('profileDetail.careerEducation')} icon={<GraduationCapIcon size={18} color="#64748b" />}>
                            <DetailRow label={t('profileDetail.profession')} value={profile.specialty} />
                            <DetailRow label={t('profileDetail.employer')} value={profile.hospital || t('profileDetail.privatePractice')} />
                            <DetailRow label={t('profileDetail.education')} value={profile.education || t('profileDetail.medicalDegree')} />
                            <DetailRow label={t('profileDetail.annualIncome')} value={profile.income || t('profileDetail.confidential')} />
                        </Section>
                         ...
                         {!editable && (
                            <View className="mt-8 mb-8">
                                <Button 
                                    ...
                                    title={requestingContact ? t('profileDetail.requesting') : t('profileDetail.viewContact')}
