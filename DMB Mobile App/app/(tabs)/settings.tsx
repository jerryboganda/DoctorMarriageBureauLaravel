...
const SettingRow = ({
    ...
    >
        <View
            className={`w-11 h-11 rounded-xl items-center justify-center mr-4 ${danger ? 'bg-red-50' : iconBgColor}`}
        >
            {icon}
        </View>
        <View className="flex-1">
            <Text
                className={`font-semibold text-[15px] ${danger ? 'text-red-600' : 'text-slate-900'}`}
            >
                {title}
            </Text>
...
export default function SettingsTab() {
    ...
    const handleLanguageToggle = async () => {
        ...
        if (I18nManager.isRTL !== needsRTL) {
            ...
            Alert.alert(
                t('common.language'),
                newLang === 'ur'
                    ? 'ایپ کو دوبارہ شروع کیا جائے گا'
                    : 'App will restart for layout change',
        ...
        try {
            ...
            if (response.data.success || response.data.result) {
                ...
                if (
                    ...
                    qrData.startsWith('data:image/svg+xml;base64,')
                ) {
        ...
        try {
            ...
            if (response.data.result) {
                ...
                Alert.alert(
                    ...
                    response.data.message || t('settings.alerts.invalidVerification'),
                );
            }
        } catch (error: any) {
            Alert.alert(
                t('settings.alerts.errorTitle'),
                error.response?.data?.message || t('settings.alerts.enable2FAFailed'),
            );
        } finally {
    ...
    const handleDisable2FA = () => {
        Alert.alert(t('settings.alerts.disable2FATitle'), t('settings.alerts.disable2FAMsg'), [
            { text: t('settings.alerts.cancel'), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert(
                            t('settings.alerts.errorTitle'),
                            t('settings.alerts.disable2FAFailed'),
                        );
                    }
                },
            },
    ...
    const handleRevokeDevice = async (deviceId: number) => {
        Alert.alert(t('settings.alerts.revokeDeviceTitle'), t('settings.alerts.revokeDeviceMsg'), [
            { text: t('settings.alerts.cancel'), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert(
                            t('settings.alerts.errorTitle'),
                            t('settings.alerts.revokeDeviceFailed'),
                        );
                    }
                },
            },
    ...
    const handleLogout = () => {
        Alert.alert(t('settings.alerts.signOutTitle'), t('settings.alerts.signOutMsg'), [
            { text: t('settings.alerts.cancel'), style: 'cancel' },
            {
                        ...
                        try {
                            ...
                            Alert.alert(
                                ...
                                error.response?.data?.message ||
                                    t('settings.alerts.deleteAccountFailed'),
                            );
                        }
                    },
                },
            ...
            <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
                ...
                >
                    ...
                    <View className="bg-white rounded-2xl px-4 mb-6 shadow-sm">
                        ...
                        <SettingRow
                            ...
                            subtitle={t('settings.account.devicesConnected', {
                        ...
                        <SettingRow
                            ...
                            subtitle={
                                i18n.language === 'ur'
                                    ? t('settings.account.languageDescUrdu')
                ...
                >
                    <View className="flex-row items-center mb-3 ml-1">
                        ...
                        <Text className="text-xs font-bold text-pink-700 uppercase tracking-wider">
                    ...
                    >
                        ...
                        <View className="items-center mb-6">
                            ...
                            <Text className="text-2xl font-bold text-slate-900">
                                {t('settings.twoFactorModal.title')}
                            </Text>
                            ...
                            {qrCode ? (
                                ...
                                <View className="w-48 h-48 bg-white rounded-xl items-center justify-center">
                                    ...
                                    <Text className="text-slate-400 text-xs mt-2">
                                        {t('settings.twoFactorModal.loading')}
                                    </Text>
                                </View>
                            )}
                            ...
                            {manualKey && (
                                <View className="mt-4 items-center">
                                    <Text className="text-xs text-slate-500 font-medium mb-1">
                                        {t('settings.twoFactorModal.manualEntryCode')}
                                    </Text>
                                    <View className="bg-slate-200 px-3 py-2 rounded-lg">
                                        <Text className="text-slate-700 font-mono text-sm tracking-widest selectable">
                                            {manualKey}
                                        </Text>
                                    </View>
                                </View>
                        ...
                        <View className="mb-6">
                            <Text className="text-xs font-bold text-slate-500 uppercase mb-2">
                                {t('settings.twoFactorModal.verificationCode')}
                            </Text>
                            <TextInput
                        ...
                        <View className="flex-row gap-3">
                            ...
                            >
                                <Text className="text-slate-700 font-bold text-lg">
                                    {t('settings.twoFactorModal.cancel')}
                                </Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                            ...
                            >
                                {enabling2FA ? (
                                    ...
                                    <Text className="text-white font-bold text-lg">
                                        {t('settings.twoFactorModal.enable')}
                                    </Text>
                                )}
                            </TouchableOpacity>
                        </View>
