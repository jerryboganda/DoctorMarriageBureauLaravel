...
const FileUploader = ({
                    ...
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="truncate">
                            ...
                            <p className="text-[10px] text-slate-500 font-medium">
                                {t('modals.verification.processingFile', 'Processing file...')}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="truncate">
                            ...
                            <p className="text-[10px] text-slate-500 font-medium">
                                ...
                                {t('modals.verification.readyToUpload')}
                            </p>
                        </div>
                    </div>
                    <button
    ...
    return (
        ...
        >
            ...
            <p
                className={`font-bold text-sm transition-colors ${isDragOver ? 'text-primary' : 'text-slate-700'}`}
            >
                {isDragOver
                    ? t('modals.verification.dropHere')
                    : t('modals.verification.clickOrDrag')}
            </p>
...
const VerificationModal: React.FC<VerificationModalProps> = ({
    ...
    const [verificationStatus, setVerificationStatus] = useState<'approved' | 'pending' | null>(
        null,
    );
        ...
        const fetchForm = async () => {
            try {
                ...
                if (Array.isArray(response.data)) {
                    ...
                    setLoadError(
                        response.data?.message || t('modals.verification.alreadySubmitted'),
                    );
                } else {
    ...
    useEffect(() => {
        if (!lockMode || verificationStatus !== 'pending') return;
        ...
        const pollApproval = async () => {
            try {
                ...
                if (!isMounted) return;
                if (res.data?.is_approved == 1 && !!res.data?.verification_info) {
                    setVerificationStatus('approved');
                    setLoadError(null);
                }
            } catch {
                // Ignore polling errors; keep lock active and retry.
            }
        };

        pollApproval();
        const intervalId = window.setInterval(pollApproval, approvalPollIntervalMs);
        return () => {
            isMounted = false;
            window.clearInterval(intervalId);
        };
    }, [lockMode, verificationStatus, approvalPollIntervalMs, onApproved]);

    useEffect(() => {
        if (!lockMode || verificationStatus !== 'approved') return;
        onApproved?.();
    }, [lockMode, verificationStatus, onApproved]);
    ...
    const getMissingRequiredFileIndexes = () => {
        return fileFields
            ...
            .filter(
                (index) =>
                    index >= 0 && !isOptionalVerificationField(fields[index]) && !values[index],
            );
    };

    const handleSubmit = async () => {
        try {
            const missingRequiredFiles = getMissingRequiredFileIndexes();
            if (missingRequiredFiles.length > 0) {
                setSubmitError(
                    t(
                        'modals.verification.uploadRequiredDocs',
            ...
            if (
                ...
                response.data?.verification_status === 'approved'
            ) {
                setVerificationStatus('approved');
            ...
            if (lockMode) {
                setVerificationStatus('pending');
                setLoadError(t('modals.verification.submittedDesc'));
            } else {
                setStep(4);
            }
        } catch (err: any) {
            if (err.response?.status === 413) {
                setSubmitError(
                    t(
                        'modals.verification.fileTooLarge',
                        'Uploaded files are too large. Please upload smaller files and try again.',
                    ),
                );
            } else {
                setSubmitError(
                    err.response?.data?.message || t('modals.verification.submissionFailed'),
                );
            }
        } finally {
            setSubmitting(false);
            ...
            return (
                <div className="flex flex-wrap gap-3">
                    {options.map((opt) => (
                        <label
                            key={opt}
                            className={`flex items-center gap-2 text-sm px-4 py-2 rounded-full border transition-all cursor-pointer ${value === opt ? 'border-primary bg-primary/5 text-primary font-bold' : 'border-slate-200 hover:border-slate-300 text-slate-600'}`}
                        >
                            <input
                                type="radio"
                                name={`field_${index}`}
        ...
        if (field.type === 'multi_select') {
            return (
                <select
                    ...
                    onChange={(e) => {
                        const selected = Array.from<HTMLOptionElement>(
        ...
        if (field.type === 'file') {
            return (
                <FileUploader
                    label={field.label}
                    value={values[index]}
                    isProcessing={!!processingFiles[index]}
                    onChange={async (file) => {
                        setSubmitError(null);
                        if (!file) {
                ...
                <div className="flex-1 overflow-y-auto px-8 py-6">
                    <div className="w-full mb-8">
                        <div className="flex justify-between relative">
                            ...
                            <div
                                ...
                                style={{ right: `${100 - ((step - 1) / 3) * 100}%` }}
                            ></div>
                            {[
                                t('modals.verification.stepInfo'),
                                t('modals.verification.stepDocuments'),
                                t('modals.verification.stepReview'),
                                t('modals.verification.stepComplete'),
                            ].map((label, idx) => {
                                const s = idx + 1;
                                ...
                                return (
                                    <div
                                        key={label}
                                        className="flex flex-col items-center gap-2 bg-white px-2 cursor-default"
                                    >
                                        <div
                                            className={`size-8 rounded-full flex items-center justify-center text-xs font-bold border-[3px] transition-all duration-300 ${isActive ? 'bg-primary border-primary text-white shadow-lg shadow-primary/30 scale-110' : 'bg-white border-slate-200 text-slate-300'}`}
                                        >
                                            {isCompleted ? <Check size={14} /> : s}
                                        </div>
                                        <span
                                            className={`text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 ${isActive ? 'text-primary' : 'text-slate-300'}`}
                                        >
                                            {label}
                                        </span>
                                    </div>
                    ...
                    {loading && (
                        <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                            <LoadingTimeoutFallback
                                message={t('modals.verification.loading')}
                                timeoutMs={10000}
                    ...
                    {loadError && !loading && verificationStatus === 'approved' && (
                        <div className="bg-emerald-50 p-8 rounded-2xl flex flex-col items-center text-center gap-3 text-sm text-emerald-800 border border-emerald-200">
                            ...
                            <h3 className="font-bold text-xl text-emerald-900">
                                {t('modals.verification.identityVerified')}
                            </h3>
                            <p className="max-w-sm mx-auto text-emerald-700/80 leading-relaxed">
                                {loadError}
                            </p>
                            <div className="mt-2 px-4 py-1.5 bg-emerald-100 rounded-full">
                                <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">
                                    {t('modals.verification.verifiedMember')}
                                </span>
                            </div>
                            {!lockMode && (
                                <button
                                    onClick={safeClose}
                                    className="mt-4 px-8 py-2.5 bg-emerald-600 text-white rounded-full font-bold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition-colors"
                                >
                    ...
                    {loadError && !loading && verificationStatus === 'pending' && (
                        <div className="bg-amber-50 p-8 rounded-2xl flex flex-col items-center text-center gap-3 text-sm text-amber-800 border border-amber-200">
                            ...
                            <h3 className="font-bold text-xl text-amber-900">
                                {t('modals.verification.underReview')}
                            </h3>
                            <p className="max-w-sm mx-auto text-amber-700/80 leading-relaxed">
                                {loadError}
                            </p>
                            <div className="mt-2 px-4 py-1.5 bg-amber-100 rounded-full">
                                <span className="text-xs font-bold text-amber-700 uppercase tracking-wider">
                                    {t('modals.verification.pendingReview')}
                                </span>
                            </div>
                            {!lockMode && (
                                <button
                                    onClick={safeClose}
                                    className="mt-4 px-8 py-2.5 bg-amber-600 text-white rounded-full font-bold shadow-lg shadow-amber-600/20 hover:bg-amber-700 transition-colors"
                                >
                                    {t('modals.verification.done')}
                                </button>
                            )}
                    ...
                    {loadError && !loading && !verificationStatus && (
                        <div className="bg-red-50 p-6 rounded-2xl flex flex-col items-center text-center gap-3 text-sm text-red-800 border border-red-100">
                            ...
                            <h3 className="font-bold text-lg text-red-900">
                                {t('modals.verification.unableToStart')}
                            </h3>
                            <p className="max-w-xs mx-auto text-red-700/80">{loadError}</p>
                            {lockMode && (
                                <button
                                    onClick={() => window.location.reload()}
                                    className="mt-4 px-6 py-2 bg-red-600 text-white rounded-full font-bold shadow-sm hover:bg-red-700"
                                >
                            ...
                            {!lockMode && (
                                <button
                                    onClick={safeClose}
                                    className="mt-4 px-6 py-2 bg-white border border-red-200 rounded-full font-bold text-red-700 shadow-sm hover:bg-red-50"
                                >
                                    {t('modals.verification.done')}
                                </button>
                            )}
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="text-center mb-2">
                                <h3 className="text-xl font-bold text-slate-900">
                                    {t('modals.verification.personalInfo')}
                                </h3>
                                <p className="text-slate-500 text-sm mt-1">
                                    {t('modals.verification.confirmDetails')}
                                </p>
                            </div>
                            {infoFields.length === 0 ? (
                                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                                    ...
                                    <p className="font-bold text-slate-700">
                                        {t('modals.verification.allInfoVerified')}
                                    </p>
                                    <p className="text-xs text-slate-400 mt-1">
                                        {t('modals.verification.noAdditionalDetails')}
                                    </p>
                                </div>
                            ) : (
                            ...
                            <div className="pt-4">
                                <button
                                    onClick={() => setStep(2)}
                                    className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-slate-800 shadow-xl shadow-slate-900/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group"
                                >
                    ...
                    {!loading && !loadError && step === 2 && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="text-center mb-2">
                                <h3 className="text-xl font-bold text-slate-900">
                                    {t('modals.verification.documentUpload')}
                                </h3>
                                <p className="text-slate-500 text-sm mt-1">
                                    {t('modals.verification.uploadDocsDesc')}
                                </p>
                            </div>
                            {fileFields.length === 0 ? (
                                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                                    ...
                                    <p className="font-bold text-slate-700">
                                        {t('modals.verification.docsNotRequired')}
                                    </p>
                                    <p className="text-xs text-slate-400 mt-1">
                                        {t('modals.verification.noUploadsNeeded')}
                                    </p>
                                </div>
                            ) : (
                                    ...
                                    {fileFields.map((field, idx) => (
                                        <div key={`${field.label}-${idx}`} className="space-y-2">
                                            <div className="flex justify-between items-baseline">
                                                ...
                                                <span
                                                    className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${isOptionalVerificationField(field) ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}
                                                >
                                                    {isOptionalVerificationField(field)
                                                        ? t(
                                                              'modals.verification.optional',
                                                              'Optional',
                                                          )
                                                        : t('modals.verification.required')}
                                                </span>
                                            </div>
                                            {renderField(field, fields.indexOf(field))}
                                        </div>
                            ...
                            <div className="pt-4 flex gap-3">
                                <button
                                    onClick={() => setStep(1)}
                                    className="px-6 py-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors"
                                >
                                ...
                                <button
                                    ...
                                    className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-slate-800 shadow-xl shadow-slate-900/10 active:scale-[0.98] transition-all"
                                >
                    ...
                    {!loading && !loadError && step === 3 && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="text-center mb-2">
                                <h3 className="text-xl font-bold text-slate-900">
                                    {t('modals.verification.confirmDetailsLabel')}
                                </h3>
                                <p className="text-slate-500 text-sm mt-1">
                                    {t('modals.verification.doubleCheck')}
                                </p>
                            </div>
                                            ...
                                            {field.type === 'file' ? (
                                                values[index] ? (
                                                    ...
                                                    >
                                                        {isOptionalVerificationField(field)
                                                            ? t(
                                                                  'modals.verification.optionalNotProvided',
                        ...
                        <div className="space-y-6 animate-in fade-in zoom-in duration-500 text-center py-12">
                            ...
                            <div className="space-y-2">
                                <h3 className="text-2xl font-bold text-slate-900">
                                    {t('modals.verification.verificationSubmitted')}
                                </h3>
                            ...
                            <div className="pt-8">
                                <button
                                    onClick={safeClose}
                                    className="bg-slate-900 text-white px-10 py-4 rounded-full font-bold hover:bg-slate-800 shadow-xl shadow-slate-900/10 transition-all active:scale-[0.98]"
                                >
