...
import { UserIcon, CalendarIcon, BadgeIcon, StethoscopeIcon, MailIcon, LockIcon, ChevronLeftIcon, CaduceusIcon, CheckIcon, PhoneIcon, SmartphoneIcon } from './Icons';
          ...
          {step === 0 && (
            ...
            >
              <div className="pt-[110px] px-6 pb-40 min-h-full">
                ...
                >
                  <h2 className="text-3xl font-extrabold text-slate-900 mb-3 tracking-tight">Who are you?</h2>
                  <p className="text-slate-500 text-lg leading-relaxed mb-8">Let's verify your identity to begin the matchmaking process.</p>
                </motion.div>

                <div className="space-y-6">
                  <Input id="fullname" label="Full Name" icon={<UserIcon className="w-5 h-5" />} />
                  ...
                  <div className="pt-2">
                      <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1 mb-4 block">Gender Identity</label>
                          ...
                          {['Male', 'Female'].map((option) => {
                              ...
                              return (
                                  <button 
                                      key={option}
                                      type="button"
                                      onClick={() => setGender(option.toLowerCase() as any)}
                                  ...
                                  >
                                      ...
                                      {!isSelected && <div className="absolute inset-0 bg-slate-50 opacity-0 group-hover:opacity-100 transition-opacity" />}
                                  </button>
                              );
                          })}
                      </div>
              ...
              <div className="pt-[110px] px-6 pb-40 min-h-full">
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                      <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Credentials</h2>
                  ...
                  <p className="text-slate-500 text-lg leading-relaxed">Your medical license will be verified against the national registry.</p>
                </motion.div>

                <div className="space-y-6">
                  <Input id="license" label="Medical License Number" icon={<BadgeIcon className="w-5 h-5" />} placeholder="MD-2024-XXXX" />
                  <Input id="specialization" label="Specialization" icon={<StethoscopeIcon className="w-5 h-5" />} placeholder="e.g. Cardiology" />
                  <Input id="hospital" label="Hospital / Practice" icon={<StethoscopeIcon className="w-5 h-5" />} placeholder="e.g. City General" />
                </div>
              </div>
            </motion.div>
          )}
            ...
            >
              <div className="pt-[110px] px-6 pb-40 min-h-full">
                <motion.div
                  ...
                  className="mb-8"
                >
                  <h2 className="text-3xl font-extrabold text-slate-900 mb-3 tracking-tight">Secure Access</h2>
                  <p className="text-slate-500 text-lg leading-relaxed">Create a secure login and add a verified phone number.</p>
                </motion.div>

                <form onSubmit={handleSubmit} className="space-y-6">
                  <Input id="reg-email" type="email" label="Email Address" icon={<MailIcon className="w-5 h-5" />} />
                  ...
                  <Input 
                    ...
                    value={phoneNumber}
                    onChange={(e: any) => setPhoneNumber((e.target as any).value)}
                  />

                  <Input id="reg-pass" type="password" label="Create Password" icon={<LockIcon className="w-5 h-5" />} />
                  <Input id="reg-confirm" type="password" label="Confirm Password" icon={<LockIcon className="w-5 h-5" />} />
                  ...
                  >
                      <p className="text-xs text-blue-800 leading-relaxed text-center font-medium">
                          By applying, you confirm you are a registered medical practitioner and agree to our <span className="font-bold cursor-pointer hover:underline text-brand-blue">Terms of Service</span>.
                      </p>
                  </motion.div>
                </form>
              </div>
          ...
          {step === 3 && (
            ...
            >
              <div className="pt-[110px] px-6 pb-40 min-h-full flex flex-col items-center">
                ...
                <h2 className="text-2xl font-extrabold text-slate-900 mb-2 text-center">Verify It's You</h2>
                <p className="text-slate-500 text-center text-sm mb-8 leading-relaxed max-w-[280px]">
                    We sent a 4-digit code to <span className="font-bold text-slate-900">{phoneNumber || "your phone"}</span>. Enter it below to complete registration.
                </p>

                <OtpInput onComplete={handleVerifyOtp} />

                {isLoading && <p className="text-sm text-brand-blue font-bold mt-8 animate-pulse">Verifying code...</p>}
                
                <p className="text-xs text-slate-400 mt-8">Didn't receive code? <button className="font-bold text-brand-blue">Resend</button></p>
              </div>
            </motion.div>
          )}
