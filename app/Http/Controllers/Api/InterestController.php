...
class InterestController extends Controller
{
    ...
    /**
     * Remove/Cancel an accepted proposal between current user and target user
     */
    public function remove_accepted_interest(Request $request)
    {
        $authId = (int) auth()->id();
        $targetUserId = (int) ($request->user_id ?? $request->id ?? 0);

        if ($targetUserId <= 0) {
            $interestId = (int) ($request->interest_id ?? 0);
            if ($interestId > 0) {
                $interest = ExpressInterest::find($interestId);
                if ($interest) {
                    $targetUserId = (int) ($interest->interested_by == $authId ? $interest->user_id : $interest->interested_by);
                }
            }
        }

        if ($targetUserId <= 0) {
            return $this->failure_message('Invalid target profile selected.');
        }

        $deleted = ExpressInterest::where(function ($q) use ($authId, $targetUserId) {
            $q->where('user_id', $targetUserId)->where('interested_by', $authId);
        })->orWhere(function ($q) use ($authId, $targetUserId) {
            $q->where('user_id', $authId)->where('interested_by', $targetUserId);
        })->delete();

        if ($deleted === 0) {
            return $this->failure_message('No active proposal found to remove.');
        }

        return $this->success_message('Accepted proposal removed successfully.');
    }
}
