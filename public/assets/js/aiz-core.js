...
(function ($) {
    ...
    // Debug: Log file base URL
    console.log('File Base URL:', AIZ.data.fileBaseUrl);
    AIZ.uploader = {
        data: {
            selectedFiles: [],
            selectedFilesObject: [],
                ...
                if (data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (data[i].type === "image") {
                            var imageUrl = AIZ.data.fileBaseUrl + data[i].file_name;
                            console.log('Loading image:', imageUrl, 'File data:', data[i]); // Debug log
                            thumb =
                                '<img src="' +
                                imageUrl +
                                '" class="img-fit" onerror="console.log(\'Failed to load image:\', this.src); this.style.display=\'none\'; this.nextElementSibling.style.display=\'block\';" onload="console.log(\'Image loaded successfully:\', this.src)">' +
                                '<div style="display:none; text-align:center; padding:20px; color:#999;"><i class="la la-image" style="font-size:24px;"></i><br>Image not found</div>';
                        } else {
        ...
        trigger: function (
            ...
            $.post(
                ...
                function (data) {
                    ...
                    AIZ.plugins.aizUppy();
                    console.log('Modal opened, loading files...'); // Debug log
                    AIZ.uploader.getAllUploads(
                        AIZ.data.appUrl + "/aiz-uploader/get_uploaded_files",
                        null,
                        $('[name="aiz-uploader-sort"]').val()
