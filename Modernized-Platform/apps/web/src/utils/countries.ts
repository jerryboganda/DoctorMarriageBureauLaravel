...
export const getDefaultCountry = (): Country => {
    if (typeof window !== 'undefined' && window.localStorage) {
        try {
            const saved = window.localStorage.getItem('dmb_selected_country');
            if (saved) {
    ...
    // Try to find Pakistan as default
    const pk = countries.find((c) => c.code === 'PK');
    return pk || countries[0];
};

export const saveSelectedCountry = (country: Country) => {
    if (typeof window !== 'undefined' && window.localStorage) {
        try {
            window.localStorage.setItem('dmb_selected_country', JSON.stringify(country));
        } catch (e) {
            console.error('Failed to save selected country', e);
        }
    }
};
