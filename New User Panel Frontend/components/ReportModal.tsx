...
const ReportModal: React.FC<ReportModalProps> = ({
    ...
    const reportReasons = [
        {
            id: 'fake',
            label: t('modals.report.fakeProfile'),
            desc: t('modals.report.fakeProfileDesc'),
        },
        {
            id: 'harassment',
            label: t('modals.report.harassment'),
            desc: t('modals.report.harassmentDesc'),
        },
        { id: 'spam', label: t('modals.report.spam'), desc: t('modals.report.spamDesc') },
        {
            id: 'inappropriate',
            label: t('modals.report.inappropriate'),
            desc: t('modals.report.inappropriateDesc'),
        },
        {
            id: 'underage',
            label: t('modals.report.underage'),
            desc: t('modals.report.underageDesc'),
        },
    ];
                ...
                <div className="h-16 border-b border-red-100 bg-red-50 flex items-center justify-between px-6">
                    <div className="flex items-center gap-2 text-red-700">
                        ...
                        <h2 className="text-lg font-bold">
                            {t('modals.report.title', { name: userName })}
                        </h2>
                    </div>
                    <button onClick={onClose}>
                        <X size={20} className="text-slate-400 hover:text-red-700" />
                    </button>
                </div>
                    ...
                    {!reason ? (
                            ...
                            {reportReasons.map((r) => (
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-bold text-slate-800 group-hover:text-red-800">
                                            {r.label}
                                        </span>
                                        <ChevronRight
                                            size={16}
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right duration-300">
                            <button
                                ...
                                className="text-xs font-bold text-slate-500 hover:text-slate-800 mb-2"
                            >
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-900 mb-2">
                                    {t('modals.report.attachEvidence')}{' '}
                                ...
                                <label className="flex items-center gap-3 cursor-pointer">
                                    ...
                                    <div>
                                        <span className="block text-sm font-bold text-slate-900">
                                            {t('modals.report.blockUser', { name: userName })}
                                        </span>
                                        <span className="text-xs text-slate-500">
                                            {t('modals.report.blockDesc')}
                                        </span>
                                    </div>
                                </label>
                            </div>
