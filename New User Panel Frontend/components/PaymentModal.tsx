...
import { X, Lock, CreditCard, Wallet, Building, CheckCircle2, ShieldCheck, Loader2 } from 'lucide-react';
...
const PaymentModal: React.FC<PaymentModalProps> = ({ itemId, itemName, amount, purchaseType, appliedCouponCode, onCouponApplied, onClose }) => {
  ...
  const [couponData, setCouponData] = useState<{ code: string; discount_amount: number; final_amount: number } | null>(null);
  ...
  const purchaseLabel = purchaseType === 'package' ? t('billing.payment.packagePurchase') : t('billing.payment.addonPurchase');
  const successNote = purchaseType === 'package'
  ...
  useEffect(() => {
    ...
    const fetchMethods = async () => {
      try {
        ...
        const response = await api.get('/payment-types', { params: { payment_type: purchaseType === 'addon' ? 'addon_payment' : 'package_payment' } });
        if (!isActive) return;
  ...
  const selectedMethod = useMemo(() => {
    ...
    return paymentMethods.find((method) => (method.payment_type_key || method.payment_type) === selectedKey) || null;
  ...
  const handlePay = async () => {
    ...
    if (isManual) {
      ...
      try {
        ...
        formData.append(purchaseType === 'package' ? 'package_id' : 'addon_id', String(itemId));
        ...
        const response = await api.post(purchaseType === 'package' ? '/member/package-purchase' : '/member/addon-purchase', formData, {
    ...
    if (isWallet) {
      try {
        setSubmitting(true);
        const response = await api.post(purchaseType === 'package' ? '/member/package-purchase' : '/member/addon-purchase', {
    ...
    window.open(`${api.defaults.baseURL}/${gatewayPath}?${params.toString()}`, '_blank', 'noopener');
  };
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-8 text-center animate-in zoom-in-95">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">{t('billing.payment.submitted')}</h2>
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-8 text-center animate-in zoom-in-95">
          ...
          <h2 className="text-xl font-bold text-slate-900 mb-2">{t('billing.payment.completePayment')}</h2>
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-full max-h-[90vh] md:h-[600px] animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="w-full md:w-5/12 bg-slate-50 p-6 md:p-8 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col shrink-0">
          <div className="flex justify-between items-center md:block mb-4 md:mb-6">
            <h3 className="font-bold text-slate-900 text-lg flex items-center gap-2">
              <ShieldCheck className="text-green-600" size={20} /> {t('billing.payment.orderSummary')}
            </h3>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-start">
              ...
              <p className="font-bold text-slate-900">Rs.{subtotal.toLocaleString()}</p>
            </div>

            {discountAmount > 0 && couponData ? (
              <div className="flex justify-between items-center text-sm text-green-600">
                <span>{t('billing.payment.discount')} ({couponData.code})</span>
            ...
            <div className="flex justify-between items-center">
              <p className="font-bold text-lg text-slate-900">{t('billing.payment.total')}</p>
              <p className="font-black text-2xl text-primary">Rs.{total.toLocaleString()}</p>
            </div>

            <div className="pt-2 space-y-2">
              <p className="text-xs font-semibold text-slate-500">{t('billing.payment.haveCoupon')}</p>
              <div className="flex gap-2">
                ...
                >
                  {couponStatus === 'loading' ? t('billing.payment.applying') : t('billing.payment.apply')}
                </button>
              </div>
              {couponMessage && (
                <p className={`text-xs ${couponStatus === 'error' ? 'text-red-600' : 'text-green-600'}`}>
                  {couponMessage}
                </p>
              )}
              {couponData && (
                <button onClick={clearCoupon} className="text-xs text-slate-500 hover:text-slate-700 underline">
        ...
        <div className="w-full md:w-7/12 p-6 md:p-8 flex flex-col relative overflow-y-auto">
          <button onClick={onClose} className="hidden md:block absolute right-6 top-6 text-slate-400 hover:text-slate-600"><X size={20} /></button>

          <h2 className="text-2xl font-bold text-slate-900 mb-6">{t('billing.payment.paymentMethod')}</h2>
            ...
            <div className="space-y-3 mb-8">
              {paymentMethods.map((method) => (
                <PaymentOption
                  ...
                  icon={method.payment_type === 'manual_payment' ? <Building size={20} /> : method.payment_type_key === 'wallet' ? <Wallet size={20} /> : <CreditCard size={20} />}
                  label={method.title || method.name}
                  selected={selectedKey === (method.payment_type_key || method.payment_type)}
                  onSelect={() => setSelectedKey(method.payment_type_key || method.payment_type)}
                />
              ))}
            </div>
          )}
          ...
          {selectedMethod?.payment_type === 'manual_payment' && (
            <div className="space-y-4 mb-8 animate-in fade-in">
              {selectedMethod.details && (
                <div className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded-lg p-3" dangerouslySetInnerHTML={{ __html: selectedMethod.details }} />
              )}
              <input
                type="text"
          ...
          <div className="mt-auto">
            ...
            >
              {submitting ? t('billing.payment.processing') : (
                <>
                  <Lock size={20} /> {t('billing.payment.payAmount', { amount: total.toLocaleString() })}
                </>
              )}
            </button>
            <div className="flex justify-center items-center gap-4 mt-4 text-xs text-slate-400 grayscale opacity-70">
              <ShieldCheck size={12} /> <span className="font-medium">{t('billing.payment.sslSecure')}</span>
            </div>
          </div>
        </div>
      </div>
...
const PaymentOption: React.FC<{ icon: React.ReactNode, label: string, selected: boolean, onSelect: () => void }> = ({ icon, label, selected, onSelect }) => (
  <div
    onClick={onSelect}
