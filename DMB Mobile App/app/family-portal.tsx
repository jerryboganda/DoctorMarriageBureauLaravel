...
import { View, Text, ScrollView, TouchableOpacity, Image, RefreshControl, ActivityIndicator, Alert, Share, TextInput } from 'react-native';
...
import {
    UsersIcon, ShieldIcon, FileTextIcon, QrCodeIcon, ChevronLeftIcon,
    CheckCircleIcon, XCircleIcon, PlusIcon, TrashIcon, ShareIcon, InfoIcon
...
export default function FamilyPortalScreen() {
    ...
    const handleUpdateProfile = async () => {
        try {
            ...
            Alert.alert(t('common.error'), error.response?.data?.message || t('familyPortal.updateFailed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const tabs = [
        ...
        { id: 'approvals', label: 'Approvals', icon: CheckCircleIcon, badge: familyData?.approvals.filter(a => !a.approved).length },
        ...
        switch (activeTab) {
                    ...
                    <View className="p-4">
                        <View className="bg-white p-5 rounded-2xl border border-slate-100 mb-4">
                            <View className="flex-row justify-between items-center mb-4">
                                <Text className="text-lg font-bold text-slate-900">{t('familyPortal.familyOverview')}</Text>
                                <TouchableOpacity
                                    onPress={() => setEditingProfile(!editingProfile)}
                                    className="px-3 py-1 bg-slate-100 rounded-full"
                                >
                                    <Text className="text-xs font-bold text-slate-600">{editingProfile ? t('common.cancel') : t('common.edit')}</Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            {editingProfile ? (
                                <View className="space-y-4">
                                    <View>
                                        <Text className="text-xs font-bold text-slate-500 mb-1">{t('familyPortal.aboutFamily')}</Text>
                                        <TextInput
                                            value={profileForm.description}
                                            onChangeText={(t) => setProfileForm(prev => ({ ...prev, description: t }))}
                                            multiline
                                    ...
                                    <View>
                                        <Text className="text-xs font-bold text-slate-500 mb-1">{t('familyPortal.traditionLevel')}</Text>
                                        <TextInput
                                            value={profileForm.traditionLevel}
                                            onChangeText={(t) => setProfileForm(prev => ({ ...prev, traditionLevel: t }))}
                                            className="bg-slate-50 p-3 rounded-xl text-slate-900"
                                        />
                                    </View>
                                    <Button onPress={handleUpdateProfile} title={t('common.saveChanges')} />
                                </View>
                            ) : (
                                <View>
                                    <Text className="text-slate-600 leading-6 mb-4">
                                        {familyData?.profile?.description || t('familyPortal.noDescription')}
                                    </Text>
                                    <View className="flex-row gap-4">
                                        <View className="bg-purple-50 px-3 py-2 rounded-lg">
                                            <Text className="text-xs text-purple-500 font-bold mb-0.5">{t('familyPortal.tradition')}</Text>
                                            <Text className="text-sm font-bold text-purple-800">{familyData?.profile?.traditionLevel}</Text>
                                        </View>
                                        <View className="bg-emerald-50 px-3 py-2 rounded-lg">
                                            <Text className="text-xs text-emerald-500 font-bold mb-0.5">{t('familyPortal.affluence')}</Text>
                                            <Text className="text-sm font-bold text-emerald-800">{familyData?.profile?.affluenceLevel}</Text>
                                        </View>
                                    </View>
                                </View>
                            )}
            ...
            case 'guardians':
                return (
                    <View className="p-4">
                        <View className="flex-row justify-between items-center mb-4">
                            <Text className="font-bold text-lg text-slate-900 ml-1">{t('familyPortal.familyMembers')}</Text>
                            <TouchableOpacity
                            ...
                            >
                                ...
                                <Text className="text-xs font-bold text-blue-600 ml-1">{t('familyPortal.invite')}</Text>
                            </TouchableOpacity>
                        </View>

                        {familyData?.guardians.map((g, i) => (
                            <View key={g.id} className="bg-white p-4 rounded-xl border border-slate-100 mb-3 flex-row items-center">
                                <View className="w-10 h-10 bg-orange-100 rounded-full items-center justify-center mr-3">
                                    <Text className="text-orange-600 font-bold">{g.name[0]}</Text>
                                </View>
                                <View className="flex-1">
                                    <Text className="font-bold text-slate-900">{g.name} {g.isOwner && '(Owner)'}</Text>
                                    <Text className="text-xs text-slate-500">{g.role} • {g.status}</Text>
                                </View>
                                {!g.isOwner && (
            ...
            case 'approvals':
                return (
                    <View className="p-4">
                        <Text className="font-bold text-lg text-slate-900 mb-4 ml-1">{t('familyPortal.pendingApprovals')}</Text>
                        {familyData?.approvals.length === 0 ? (
                            <View className="items-center py-8">
                                <CheckCircleIcon size={40} color="#cbd5e1" />
                                <Text className="text-slate-400 mt-2">{t('familyPortal.noPendingApprovals')}</Text>
                            </View>
                        ) : (
                            familyData?.approvals.map((approval) => (
                                <View key={approval.id} className="bg-white p-4 rounded-xl border border-slate-100 mb-3">
                                    <View className="flex-row items-center mb-3">
                                        <Image source={{ uri: approval.img }} className="w-12 h-12 rounded-full mr-3 bg-slate-200" />
                                        <View>
                                            <Text className="font-bold text-slate-900">{approval.name}</Text>
                                            <Text className="text-xs text-slate-500">{approval.time}</Text>
                                        </View>
                                    </View>
                                    ...
                                    <View className="flex-row gap-3">
                                        ...
                                        <TouchableOpacity className="flex-1 h-9 items-center justify-center border border-slate-200 rounded-xl">
                                            <Text className="font-bold text-slate-600">{t('familyPortal.discuss')}</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            ))
                ...
                return (
                    <View className="p-6 items-center">
                        <View className="bg-white p-6 rounded-3xl border border-slate-100 w-full items-center shadow-sm">
                            ...
                            <Text className="mt-4 text-xl font-bold text-slate-900">{user?.name}</Text>
        ...
        <View className="flex-1 bg-slate-50">
            ...
            >
                <View className="flex-row items-center py-4">
                    ...
                    <View>
                        <Text className="text-white text-lg font-bold">{t('familyPortal.title')}</Text>
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <Text className={`text-xs font-bold ${isActive ? 'text-blue-900' : 'text-white'}`}>
                                    {tab.label}
                                </Text>
                                {tab.badge ? (
