...
const buildSnapshot = (
    ...
    const state = normalizeState(
        readFirst(profile, [
            ...
            kind === 'profile_photo' ? 'photoRequestState' : 'galleryImageRequestState',
    ...
    const requested = truthy(
        readFirst(profile, [
            ...
            kind === 'profile_photo' ? 'photoRequestRequested' : 'galleryImageRequestRequested',
    ...
    const approved = truthy(
        readFirst(profile, [
            ...
            kind === 'profile_photo' ? 'photoRequestApproved' : 'galleryImageRequestApproved',
    ...
    const rawRequired = truthy(
        readFirst(profile, [
            `${prefix}RequestRequired`,
            `${prefix}_request_required`,
            kind === 'profile_photo' ? 'photoRequestRequired' : 'galleryImageRequestRequired',
    ...
    const accessible = truthy(
        readFirst(profile, [
            `${prefix}Accessible`,
            `${prefix}_accessible`,
            kind === 'profile_photo' ? 'photoAccessible' : 'galleryImageAccessible',
    ...
    const hasGalleryArray =
        (Array.isArray(profile?.photo_gallery) && profile.photo_gallery.length > 0) ||
        (Array.isArray(profile?.gallery_images) && profile.gallery_images.length > 0) ||
        (Array.isArray(profile?.gallery) && profile.gallery.length > 0) ||
        (Array.isArray(profile?.photos) && profile.photos.length > 0);

    const hasGalleryCount =
        Number(
            profile?.gallery_count ?? profile?.galleryCount ?? profile?.photo_gallery_count ?? 0,
        ) > 0;

    const rawExists = readFirst(profile, [
        `${prefix}Exists`,
        `${prefix}_exists`,
        kind === 'profile_photo' ? 'photoExists' : 'galleryImageExists',
        kind === 'profile_photo' ? 'photoExists' : 'galleryExists',
        kind === 'profile_photo' ? 'photo_exists' : 'gallery_exists',
        kind === 'profile_photo' ? 'profile_photo_exists' : 'has_gallery_images',
        'has_gallery',
        'has_photo_gallery',
    ]);
    ...
    const exists =
        kind === 'gallery_image'
            ? hasGalleryArray ||
              ...
              truthy(accessible) ||
              state !== 'none'
            : truthy(rawExists) || truthy(rawRequired) || truthy(accessible) || state !== 'none';

    const required = rawRequired || (exists && !accessible);

    const requestId =
        readFirst(profile, [
            `${prefix}RequestId`,
            `${prefix}_request_id`,
            kind === 'profile_photo' ? 'photoRequestId' : 'galleryImageRequestId',
    ...
    const text = `${
        readFirst(profile, [
            ...
            kind === 'profile_photo' ? 'photoRequestText' : 'galleryImageRequestText',
