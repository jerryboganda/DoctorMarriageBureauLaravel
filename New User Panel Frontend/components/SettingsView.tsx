...
const SettingsView: React.FC<SettingsViewProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'account' | 'privacy' | 'safety' | 'billing'>(
        'account',
    );
    const [loading, setLoading] = useState(true);
    const [managementMode, setManagementMode] = useState<'self' | 'family' | 'matchmaker' | 'dual'>(
        'self',
    );
    ...
    const [visibilityStatus, setVisibilityStatus] = useState<'idle' | 'saving' | 'saved' | 'error'>(
        'idle',
    );
    ...
    const applyVisibilitySnapshot = React.useCallback(
        (visibilityData: Record<string, any>) => {
    ...
    const handleTrustedPhoneChange = (value: string) => {
        ...
        setTrustedForm((prev) => ({
            ...prev,
            phone: digits ? trustedCountry.dialCode + digits : '',
        }));
    };
    ...
    const handleTransferPhoneChange = (value: string) => {
        ...
        setTransferForm((prev) => ({
            ...prev,
            to_phone: digits ? transferCountry.dialCode + digits : '',
        }));
    };
    ...
    useEffect(() => {
        if (showTrustedForm && trustedForm.phone) {
            ...
            const localPart = oldCountry
                ? trustedForm.phone.slice(oldCountry.dialCode.length)
                : trustedForm.phone.replace(/^\+?\d*/, '');
            setTrustedForm((prev) => ({
                ...prev,
                phone: localPart ? trustedCountry.dialCode + localPart : '',
            }));
        }
    }, [trustedCountry]);
    ...
    useEffect(() => {
        if (showTransferForm && transferForm.to_phone) {
            ...
            const localPart = oldCountry
                ? transferForm.to_phone.slice(oldCountry.dialCode.length)
                : transferForm.to_phone.replace(/^\+?\d*/, '');
            setTransferForm((prev) => ({
                ...prev,
                to_phone: localPart ? transferCountry.dialCode + localPart : '',
            }));
        }
    }, [transferCountry]);
    ...
    const [safetyModalContent, setSafetyModalContent] = useState<{
        title: string;
    ...
    const handleApplyCoupon = async () => {
        ...
        try {
            ...
            setCouponStatus('error');
            setCouponMessage(
                error.response?.data?.message || t('settings.billing.failedApplyCode'),
            );
        }
    };
    ...
    const handlePhotoVisibilityChange = async (value: 'everyone' | 'requests') => {
        ...
        try {
            ...
            try {
                ...
                console.error(
                    'Failed to refresh visibility after photo visibility error',
                    refreshError,
                );
            }
            if (visibilityStatusTimerRef.current) clearTimeout(visibilityStatusTimerRef.current);
    ...
    const handleInitiateTransfer = async () => {
        if (
            !transferForm.to_name.trim() ||
            (!transferForm.to_email.trim() && !transferForm.to_phone.trim())
        ) {
            return;
        }
    ...
    const trustScore = [
        ...
        is2FAEnabled ? 35 : 0,
        verifiedContactCount > 0 ? 35 : 0,
    ].reduce((sum, val) => sum + val, 0);
    const trustLabel =
        trustScore >= 80
            ? t('settings.safety.excellent')
            : trustScore >= 60
              ? t('settings.safety.good')
              : trustScore >= 40
                ? t('settings.safety.fair')
                : t('settings.safety.needsWork');
                ...
                <div className="max-w-6xl mx-auto">
                        ...
                        (loading ? (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4">
                                ...
                                <div className="space-y-6">
                                    <SectionHeader
                                        title={t('settings.security.accessCredentials')}
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                        <div className="p-6 border-b border-slate-100">
                                            <h4 className="font-bold text-slate-900 mb-4">
                                                {t('settings.security.registrationMethods')}
                                            </h4>
                                            <div className="space-y-4">
                                                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        ...
                                                        <div className="overflow-hidden">
                                                            <p className="text-sm font-bold text-slate-900">
                                                                {t(
                                                                    'settings.security.emailAddress',
                                                                )}
                                                            </p>
                                                    ...
                                                    <span
                                                        className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full shrink-0 ${emailCredential?.verified ? 'text-green-600 bg-green-50' : 'text-slate-500 bg-slate-100'}`}
                                                    >
                                                        {emailCredential?.verified ? (
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    setShowCredentialVerification(
                                                                        true,
                                                                    );
                                                                }}
                                                                className="hover:text-primary hover:underline flex items-center gap-1"
                                                            >
                                                                {t('settings.security.verifyNow')}{' '}
                                        ...
                                        <div className="p-6 bg-slate-50/50">
                                            <div className="flex justify-between items-center mb-4">
                                                <h4 className="font-bold text-slate-900 flex items-center gap-2">
                                                    ...
                                                    {t('settings.security.accountRecovery')}
                                                </h4>
                                                ...
                                                {trustedContacts.length > 0 && (
                                                    <div className="space-y-2">
                                                        {trustedContacts.map((contact) => (
                                                            <div
                                                                key={contact.id}
                                                                className="p-3 bg-white border border-slate-200 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    <div
                                                                        className={`size-8 rounded-full flex items-center justify-center ${contact.is_verified ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}
                                                                    >
                                                                        <ShieldCheck size={16} />
                                                                    </div>
                                                                    <div>
                                                                        ...
                                                                        <p className="text-xs text-slate-500">
                                                                            ...
                                                                            {contact.email ||
                                                                                contact.phone ||
                                                                                t(
                                                                                    'settings.security.contactAdded',
                                                                                )}
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                                <div className="flex items-center gap-3">
                                                                    {!contact.is_verified && (
                                                                        ...
                                                                        >
                                                                            {t(
                                                                                'settings.security.resend',
                                                                            )}
                                                                        </button>
                                                                    )}
                                                                    <button
                                                                        onClick={() =>
                                                                            handleRemoveTrustedContact(
                                                                                contact.id,
                                                                            )
                                                                        }
                                                    ...
                                                    <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3">
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                            ...
                                                            <div>
                                                                <label className="block text-xs font-bold text-slate-700 mb-1">
                                                                    {t(
                                                                        'settings.security.relationship',
                                                                    )}
                                                                </label>
                                                                <select
                                                                ...
                                                                >
                                                                    <option value="parent">
                                                                        {t(
                                                                            'settings.security.relationParent',
                                                                        )}
                                                                    </option>
                                                                    <option value="sibling">
                                                                        {t(
                                                                            'settings.security.relationSibling',
                                                                        )}
                                                                    </option>
                                                                    <option value="spouse">
                                                                        {t(
                                                                            'settings.security.relationSpouse',
                                                                        )}
                                                                    </option>
                                                                    <option value="friend">
                                                                        {t(
                                                                            'settings.security.relationFriend',
                                                                        )}
                                                                    </option>
                                                                    <option value="relative">
                                                                        {t(
                                                                            'settings.security.relationRelative',
                                                                        )}
                                                                    </option>
                                                                    <option value="other">
                                                                        {t(
                                                                            'settings.security.relationOther',
                                                                        )}
                                                                    </option>
                                                                </select>
                                                            </div>
                                                            <div>
                                                                <label className="block text-xs font-bold text-slate-700 mb-1">
                                                                    {t(
                                                                        'settings.security.emailOptional',
                                                                    )}{' '}
                                                                    <span className="ml-1 text-[10px] font-semibold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full">
                                                                        Optional
                                                                    </span>
                                                                </label>
                                                                <input
                                                            ...
                                                            <div>
                                                                <label className="block text-xs font-bold text-slate-700 mb-1">
                                                                    {t(
                                                                        'settings.security.phoneOptional',
                                                                    )}{' '}
                                                                    <span className="ml-1 text-[10px] font-semibold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full">
                                                                        Optional
                                                                    </span>
                                                                ...
                                                                <div className="flex items-stretch bg-white border border-slate-300 rounded-lg focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all overflow-hidden">
                                                                    ...
                                                                    <input
                                                                        type="tel"
                                                                        value={
                                                                            trustedForm.phone.startsWith(
                                                                                trustedCountry.dialCode,
                                                                            )
                                                                                ? trustedForm.phone.slice(
                                                                                      trustedCountry
                                                                                          .dialCode
                                                                                          .length,
                                                                                  )
                                                                                : trustedForm.phone.replace(
                                                                                      /^\+\d+/,
                                                                                      '',
                                                                                  )
                                                                        }
                                                                        onChange={(e) =>
                                                                            handleTrustedPhoneChange(
                                                                                e.target.value,
                                                                            )
                                                                        }
                                                                        className="flex-1 bg-transparent px-3 py-2 text-sm outline-none"
                                                                        placeholder={t(
                                                                            'settings.security.phoneNumber',
                                                                        )}
                                                                    />
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="flex flex-wrap gap-2">
                                                            <button
                                                                ...
                                                                disabled={
                                                                    !trustedForm.name.trim() ||
                                                                    (!trustedForm.email.trim() &&
                                                                        !trustedForm.phone.trim())
                                                                }
                                                    ...
                                                    canAddTrustedContact && (
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-sm font-bold">
                                                                {t(
                                                                    'settings.security.addTrustedContact',
                                                                )}
                                                            </span>
                                                        </button>
                                                    )
                                        ...
                                        <div className="flex justify-between items-center mb-6">
                                            <h4 className="font-bold text-slate-900 flex items-center gap-2">
                                                <SmartphoneNfc
                                                    ...
                                                    className="text-slate-600"
                                                />{' '}
                                                {t('settings.security.deviceManagement')}
                                            </h4>
                                            {devices.length > 1 && (
                                                <button
                                        ...
                                        <div className="space-y-3">
                                            {devices.map((device) => {
                                                ...
                                                const isCurrent = Boolean(
                                                    device.is_current ?? device.current,
                                                );
                                                const deviceName =
                                                    device.device_name ??
                                                    device.name ??
                                                ...
                                                const lastActive = formatDeviceLastActive(
                                                    device.last_used_at ?? device.lastActive,
                                                );
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div
                                                                className={`size-10 rounded-full flex items-center justify-center ${isCurrent ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}
                                                            >
                                                                {deviceType === 'mobile' ? (
                                                                    <Smartphone size={20} />
                                                                ) : deviceType === 'tablet' ? (
                                                            ...
                                                            <div>
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {isCurrent && (
                                                                        <span className="text-[10px] font-bold bg-green-100 text-green-700 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                                                            {t(
                                            ...
                                            <div className="space-y-3">
                                                <label className="flex items-center justify-between cursor-pointer group">
                                                    <span className="text-sm text-slate-600 group-hover:text-slate-900">
                                                        {t('settings.security.emailAlertNewDevice')}
                                                    </span>
                                                    <input
                                                        type="checkbox"
                                ...
                                <div className="space-y-6">
                                    <SectionHeader
                                        title={t('settings.security.profileRoles')}
                                        icon={<Users size={20} className="text-primary" />}
                                    />

                                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                        <h4 className="font-bold text-slate-900 mb-4">
                                            {t('settings.security.whoManages')}
                                        </h4>
                                            ...
                                            {availableModes.length > 0 ? (
                                                ...
                                                <>
                                                    <RoleOption
                                                        ...
                                                        desc={t(
                                                            'settings.security.selfManagedDesc',
                                                        )}
                                                    ...
                                                    <RoleOption
                                                        ...
                                                        desc={t(
                                                            'settings.security.familyManagedDesc',
                                                        )}
                                                        selected={managementMode === 'family'}
                                                        onClick={() =>
                                                            handleManagementMode('family')
                                                        }
                                                    />
                                                    <RoleOption
                                                        ...
                                                        onClick={() =>
                                                            handleManagementMode('matchmaker')
                                                        }
                                                    />
                                                    <RoleOption
                                                        label={t('settings.security.dualControl')}
                                                        desc={t(
                                                            'settings.security.dualControlDesc',
                                                        )}
                                        ...
                                        <div className="border-t border-slate-100 pt-6">
                                            <h4 className="font-bold text-slate-900 mb-3">
                                                {t('settings.security.ownershipControls')}
                                            </h4>
                                            {pendingTransfer ? (
                                                <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                                                    <div className="flex items-start justify-between">
                                                        <div>
                                                            <p className="text-sm font-bold text-slate-900">
                                                                {t(
                                                                    'settings.security.transferPending',
                                                                )}
                                                            </p>
                                                            <p className="text-xs text-slate-500">
                                                                To{' '}
                                                                {pendingTransfer.to_name ||
                                                                    t(
                                                                        'settings.security.newOwner',
                                                                    )}{' '}
                                                                {pendingTransfer.to_email
                                                                    ? `(${pendingTransfer.to_email})`
                                                                    : ''}
                                                            </p>
                                                            ...
                                                            <p className="text-xs text-slate-400 mt-1">
                                                                {t(
                                                                    'settings.security.expiresInDays',
                                                                    {
                                                                        n:
                                                                            pendingTransfer.days_remaining ??
                                                                            0,
                                                                    },
                                                                )}
                                                            </p>
                                                ...
                                                <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                        <div>
                                                            <label className="block text-xs font-bold text-slate-700 mb-1">
                                                                {t(
                                                                    'settings.security.newOwnerName',
                                                                )}
                                                            </label>
                                                            <input
                                                                type="text"
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="flex items-stretch bg-white border border-slate-300 rounded-lg focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all overflow-hidden">
                                                                ...
                                                                <input
                                                                    type="tel"
                                                                    value={
                                                                        transferForm.to_phone.startsWith(
                                                                            transferCountry.dialCode,
                                                                        )
                                                                            ? transferForm.to_phone.slice(
                                                                                  transferCountry
                                                                                      .dialCode
                                                                                      .length,
                                                                              )
                                                                            : transferForm.to_phone.replace(
                                                                                  /^\+\d+/,
                                                                                  '',
                                                                              )
                                                                    }
                                                                    onChange={(e) =>
                                                                        handleTransferPhoneChange(
                                                                            e.target.value,
                                                                        )
                                                                    }
                                                                    className="flex-1 bg-transparent px-3 py-2 text-sm outline-none"
                                                        ...
                                                        <div className="sm:col-span-2">
                                                            <label className="block text-xs font-bold text-slate-700 mb-1">
                                                                {t(
                                                                    'settings.security.reasonOptional',
                                                                )}{' '}
                                                                <span className="ml-1 text-[10px] font-semibold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full">
                                                                    Optional
                                                                </span>
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        <button
                                                            ...
                                                            disabled={
                                                                transferSubmitting ||
                                                                !transferForm.to_name.trim() ||
                                                                (!transferForm.to_email.trim() &&
                                                                    !transferForm.to_phone.trim())
                                                            }
                                                            className="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-lg hover:bg-slate-800 disabled:opacity-60"
                                                        >
                                                            {transferSubmitting
                                                                ...
                                                                : t(
                                                                      'settings.security.sendTransferInvite',
                                                                  )}
                                                        </button>
                                                        <button
                                                            onClick={() =>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="text-left">
                                                            <p className="text-sm font-bold text-slate-800">
                                                                {t(
                                                                    'settings.security.transferOwnership',
                                                                )}
                                                            </p>
                                                            <p className="text-xs text-slate-500">
                                                                {t(
                                                                    'settings.security.transferOwnershipDesc',
                                                                )}
                                                            </p>
                                                        </div>
                                                    </div>
                                    ...
                                    <div
                                        className={`rounded-xl shadow-sm border p-6 transition-colors ${!is2FAEnabled ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'}`}
                                    >
                    ...
                    {activeTab === 'billing' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4">
                            <div className="space-y-6">
                                <SectionHeader
                                    title={t('settings.billing.membershipPlan')}
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                            ...
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    ...
                                                    <h3 className="text-2xl font-bold flex items-center gap-2">
                                                        {packageDetails?.name ??
                                                            t('settings.billing.basicMember')}
                                                    </h3>
                                                </div>
                                                ...
                                                {packageDetails ? (
                                                    <>
                                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                                            ...
                                                            />{' '}
                                                            {packageDetails.express_interest ?? 0}{' '}
                                                            {t('settings.billing.expressInterests')}
                                                        </div>
                                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                                            <Check
                                                                size={14}
                                                                className="text-green-400"
                                                            />{' '}
                                                            {packageDetails.contact ?? 0}{' '}
                                                            {t('settings.billing.contactViews')}
                                                        </div>
                                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                                            <Check
                                                                size={14}
                                                                className="text-green-400"
                                                            />{' '}
                                                            {packageDetails.photo_gallery ?? 0}{' '}
                                                            {t(
                                                                'settings.billing.photoGalleryViews',
                                                            )}
                                                        </div>
                                                    </>
                                                ) : (
                                                    <>
                                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                                            ...
                                                            />{' '}
                                                            {t(
                                                                'settings.billing.limitedProfileViews',
                                                            )}
                                                        </div>
                                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                                            <Check
                                                            ...
                                                            />{' '}
                                                            {t(
                                                                'settings.billing.noDirectMessaging',
                                                            )}
                                                        </div>
                                                    </>
                                                )}
                                    ...
                                    <div className="p-6 bg-white">
                                        <div className="flex items-center justify-between mb-4 text-sm text-slate-600">
                                            <span>
                                                {packageDetails?.validity ? (
                                                    <>
                                                        <b>
                                                            {t('settings.billing.validityDays', {
                                                                n: packageDetails.validity,
                                                            })}
                                                        </b>
                                                    </>
                                                ) : (
                                        ...
                                        >
                                            ...
                                            {t('settings.billing.upgradePremium')}
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-3">
                                        {purchaseHistory.length > 0 ? (
                                            purchaseHistory.map((invoice) => (
                                                <div
                                                    key={invoice.package_payment_id}
                                                    className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100 text-sm"
                                                >
                                                    <div>
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <span
                                                            className={`text-xs font-bold ${invoice.payment_status === 'Paid' ? 'text-green-600' : 'text-orange-600'}`}
                                                        >
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                    <h4 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                                        ...
                                        {t('settings.billing.addonPurchases')}
                                    </h4>
                                    <div className="space-y-3">
                                        {addonPurchaseHistory.length > 0 ? (
                                            addonPurchaseHistory.map((purchase) => (
                                                <div
                                                    key={purchase.addon_purchase_id}
                                                    className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100 text-sm"
                                                >
                                                    <div>
                                                        <p className="font-bold text-slate-900">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <span
                                                            className={`text-xs font-bold ${purchase.payment_status === 'Paid' ? 'text-green-600' : 'text-orange-600'}`}
                                                        >
                            ...
                            <div className="space-y-6">
                                    ...
                                    <div className="space-y-4 max-h-[500px] overflow-y-auto scrollbar-hide pr-1">
                                        {availablePackages.length > 0 ? (
                                            availablePackages.map((pkg) => (
                                                <div
                                                    key={pkg.package_id}
                                                    className="border border-slate-200 rounded-xl p-4 hover:border-primary/50 transition-colors relative overflow-hidden group"
                                                >
                                                    ...
                                                    <div className="relative z-10">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div>
                                                                ...
                                                                <p className="text-xs text-slate-500">
                                                                    {t(
                                                                        'settings.billing.validForDays',
                                                                        {
                                                                            n: pkg.package_active_days,
                                                                        },
                                                                    )}
                                                                </p>
                                                            </div>
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2 my-3">
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-600">
                                                                ...
                                                                />
                                                                <span>
                                                                    {pkg.express_interest}{' '}
                                                                    {t(
                                                                        'settings.billing.interests',
                                                                    )}
                                                                </span>
                                                            </div>
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-600">
                                            ...
                                            <div className="text-center py-8 text-slate-400">
                                                ...
                                                />
                                                <p className="text-xs">
                                                    {t('settings.billing.loadingPlans')}
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <SectionHeader
                                    title={t('settings.billing.paymentMethods')}
                                ...
                                {paymentMethods.length > 0 ? (
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center justify-center text-center min-h-[150px]">
                                        ...
                                        <h4 className="font-bold text-slate-900 text-sm">
                                            {t('settings.billing.noPaymentMethods')}
                                        </h4>
                                        <p className="text-xs text-slate-500 mb-0 max-w-xs">
                                            {t('settings.billing.enableGateway')}
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-blue-900 text-sm">
                                            {t('settings.billing.haveGiftCode')}
                                        </h4>
                                        <p className="text-xs text-blue-700 mb-2">
                                            {t('settings.billing.redeemOffers')}
                                        </p>
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {couponStatus === 'loading'
                                                    ? t('settings.billing.applying')
                                                    : t('settings.billing.apply')}
                                            </button>
                                        </div>
                                        {couponMessage && (
                                            <p
                                                className={`text-[11px] mt-2 ${couponStatus === 'error' ? 'text-red-600' : 'text-green-700'}`}
                                            >
                                                {couponMessage}
                                            </p>
                                        )}
                    ...
                    {activeTab === 'privacy' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4">
                            <div className="space-y-6">
                                <SectionHeader
                                    title={t('settings.privacy.visibilitySettings')}
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="font-bold text-slate-900">
                                                {t('settings.privacy.profileStatus')}
                                            </h4>
                                            <p className="text-xs text-slate-500">
                                                {t('settings.privacy.profileStatusDesc')}
                                            </p>
                                        </div>
                                        <label
                                            className={`relative inline-flex items-center ${visibilitySavingKey ? 'cursor-wait opacity-70' : 'cursor-pointer'}`}
                                        >
                                            <input
                                                type="checkbox"
                                                className="sr-only peer"
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <h4 className="font-bold text-slate-900">
                                                    {t('settings.privacy.incognitoMode')}
                                                </h4>
                                                <span className="bg-yellow-100 text-yellow-800 text-[10px] font-bold px-2 py-0.5 rounded">
                                                    {t('settings.privacy.premium')}
                                                </span>
                                            </div>
                                            <p className="text-xs text-slate-500">
                                                {t('settings.privacy.incognitoDesc')}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            {!isPremiumMember && (
                                            ...
                                            <label
                                                className={`relative inline-flex items-center ${visibilitySavingKey ? 'cursor-wait opacity-70' : isPremiumMember ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}
                                            >
                                                <input
                                                    type="checkbox"
                                                    className="sr-only peer"
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="font-bold text-slate-900">
                                                {t('settings.privacy.screenshotDeterrence')}
                                            </h4>
                                            <p className="text-xs text-slate-500">
                                                {t('settings.privacy.screenshotDesc')}
                                            </p>
                                        </div>
                                        <label
                                            className={`relative inline-flex items-center ${visibilitySavingKey ? 'cursor-wait opacity-70' : 'cursor-pointer'}`}
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="font-bold text-slate-900">
                                                {t('settings.privacy.blurProfilePicture')}
                                            </h4>
                                            <p className="text-xs text-slate-500">
                                                {t('settings.privacy.blurProfilePictureDesc')}
                                            </p>
                                        </div>
                                        <label
                                            className={`relative inline-flex items-center ${visibilitySavingKey ? 'cursor-wait opacity-70' : 'cursor-pointer'}`}
                                    ...
                                    <div>
                                        <h4 className="font-bold text-slate-900 mb-3">
                                            {t('settings.privacy.whoCanSeePhotos')}
                                        </h4>
                                        <div className="space-y-3">
                                            {(['everyone', 'requests'] as const).map((opt) => (
                                                <label
                                                    key={opt}
                                                    className={`flex items-center gap-3 p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors ${visibilitySavingKey ? 'cursor-wait opacity-70' : 'cursor-pointer'}`}
                                                >
                                                    <input
                                                        type="radio"
                                                        ...
                                                        disabled={visibilitySavingKey !== null}
                                                        onChange={() =>
                                                            handlePhotoVisibilityChange(opt)
                                                        }
                                                    />
                                                    <span className="text-sm text-slate-700 capitalize">
                                                        {opt === 'everyone'
                                                            ? 'All Members Can See'
                                                            : t('settings.privacy.onlyOnRequest')}
                                                    </span>
                                                </label>
                                            ))}
                                        </div>
                            ...
                            <div className="space-y-6">
                                <SectionHeader
                                    title={t('settings.privacy.dataBlocking')}
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-xs text-red-600/70">
                                                {t('settings.privacy.deactivateDesc')}
                                            </p>
                                        </div>
                                        <button
                    ...
                    {activeTab === 'safety' && (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4">
                            <div className="lg:col-span-2 space-y-6">
                                <SectionHeader
                                    title={t('settings.safety.identityVerification')}
                                    ...
                                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                                        ...
                                        <div className="text-center md:text-left">
                                            <h3 className="text-2xl font-bold mb-2">
                                                {t('settings.safety.getVerifiedBadge')}
                                            </h3>
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                    <h4 className="font-bold text-slate-900 mb-6">
                                        {t('settings.safety.safetyCenter')}
                                    </h4>
                            ...
                            <div className="space-y-6">
                                <SectionHeader
                                    title={t('settings.safety.communityTrust')}
                                    icon={<Star size={20} className="text-primary" />}
                                />
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 text-center">
                                    ...
                                    <h4 className="font-bold text-slate-900 text-lg">
                                        {t('settings.safety.trustScore', { score: trustScore })}
                                    </h4>
                                ...
                                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                                    <h5 className="font-bold text-slate-900 text-sm mb-3">
                                        {t('settings.safety.reportingHistory')}
                                    </h5>
                                    {tickets.length > 0 ? (
                                        <div className="space-y-2">
                                            {tickets.slice(0, 3).map((ticket: any) => (
                                                <div
                                                    key={ticket.id}
                                                    className="text-left bg-white p-2 rounded border border-slate-100"
                                                >
                                                    ...
                                                    <div className="flex justify-between items-center mt-1">
                                                        ...
                                                        <span
                                                            className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${ticket.status === 'Open' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}
                                                        >
                                                            {ticket.status}
                                                        </span>
                                                    </div>
                        ...
                        {safetyModalContent?.content === 'form' ? (
                            <div className="space-y-4">
                                ...
                                <button
                                    ...
                                    disabled={
                                        reportSubmitting || !reportSubject || !reportDescription
                                    }
                                ...
                                >
                                    {reportSubmitting ? (
                                        ...
                                        t('settings.safety.submitReport')
                                    )}
                                </button>
                            </div>
                        ) : (
                            <div>
                                ...
                                <button
                                    onClick={() => setShowSafetyModal(false)}
                                    className="mt-6 w-full py-2 bg-slate-100 text-slate-700 font-bold rounded-lg hover:bg-slate-200"
                                >
                                    {t('settings.safety.close')}
                                </button>
                            </div>
                        )}
                    </div>
...
const RoleOption: React.FC<{
    ...
    >
        <div className="flex items-center justify-between mb-1">
            <span className={`text-sm font-bold ${selected ? 'text-primary' : 'text-slate-800'}`}>
                {label}
            </span>
...
const SafetyCard: React.FC<{
    ...
    <div
        onClick={onClick}
        className="p-4 border border-slate-100 rounded-xl hover:shadow-md hover:border-primary/20 transition-all cursor-pointer group"
    >
