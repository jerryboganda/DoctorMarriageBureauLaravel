...
const ProfileDetailScreen = () => {
    ...
    const fetchProfile = useCallback(async () => {
        try {
            const response = await api.get(`/member/public-profile/${id}?read_only=true`);
            if (response.data?.result && response.data?.data) {
                setProfile(response.data.data);
            } else {
                throw new Error(response.data?.message || 'Profile is not available');
            }
        } catch (error) {
    ...
    const address = profile.present_address || {};
    ...
    const displayName = [basics.firs_name, basics.last_name].filter(Boolean).join(' ') || 'Member';
    const introduction = profile.intoduction?.introduction || t('member.noIntro');
    const location = address.city || address.state || address.country || 'N/A';
                ...
                <View className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    >
                        <View className="flex-row items-center gap-2">
                            ...
                            {profile.identity_verified && <CheckIcon size={20} color="#10b981" />}
                        </View>
                        ...
                        <View className="flex-row items-center gap-3 mt-2">
                            ...
                            <View className="flex-row items-center gap-1">
                                ...
                                <Text className="text-white/90">{location}</Text>
                            </View>
                        </View>
                    </MotiView>
                </View>
                    ...
                    <AnimatePresence exitBeforeEnter>
                        ...
                        {activeTab === 'family' && (
                            <MotiView
                                ...
                                animate={{ opacity: 1, translateX: 0 }}
                            >
                                <DetailItem label="Father" value={family.father} />
                                <DetailItem label="Mother" value={family.mother} />
                                <DetailItem label="Siblings" value={family.sibling} />
                            </MotiView>
                        )}
                    </AnimatePresence>
                    <View className="h-32" />
