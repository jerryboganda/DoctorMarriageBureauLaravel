...
import {
  BookmarkIcon, HeartIcon, TrashIcon, ChevronLeftIcon, MessageCircleIcon,
  UserIcon, MapPinIcon, BriefcaseIcon, StarIcon, XIcon, EyeIcon
...
const ShortlistScreen = () => {
  ...
  const handleRemove = async (id: number, name: string, memberId: number) => {
    Alert.alert(
      t('shortlist.removeTitle'),
      t('shortlist.removeConfirm', { name }),
      [
  ...
  const handleSendInterest = async (memberId: number, name: string) => {
    ...
    try {
      ...
      if (response.data.result) {
        ...
        Alert.alert(t('common.info'), response.data.message || t('shortlist.interestAlready'));
      }
    } catch (error: any) {
      Alert.alert(t('common.error'), error.response?.data?.message || t('shortlist.interestFailed'));
    }
  };
      ...
      <View className="flex-1 bg-slate-50">
        ...
        >
          ...
          >
            ...
            <Text className="text-white/90 mt-4 font-medium">{t('shortlist.loading')}</Text>
          </MotiView>
        </LinearGradient>
      </View>
    );
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        ...
        <View className="flex-row justify-center gap-6 pb-6">
          ...
          <View className="items-center">
            <Text className="text-3xl font-bold text-white">{shortlist.filter(s => s.verified).length}</Text>
      ...
      >
        {shortlist.length === 0 ? (
          ...
          >
            ...
            <Text className="text-xl font-bold text-slate-900 mb-2">{t('shortlist.noProfiles')}</Text>
            ...
            >
              <Text className="text-white font-bold">{t('shortlist.browseProfiles')}</Text>
            </TouchableOpacity>
          </MotiView>
        ) : (
                    ...
                    <View className="relative">
                      ...
                      {item.verified && (
                        <View className="absolute top-2 left-2 bg-emerald-500 px-2 py-1 rounded-full flex-row items-center">
                          ...
                          <Text className="text-white text-xs font-bold ml-1">{t('common.verified')}</Text>
                        </View>
                      )}

                      {/* Remove Button */}
                      <TouchableOpacity
                        onPress={() => handleRemove(item.id, item.name || 'this profile', item.user_id)}
                    ...
                    <View className="p-3">
                      ...
                      <View className="flex-row items-center mt-1">
                        ...
                        <Text className="text-slate-500 text-xs ml-1" numberOfLines={1}>
                          {item.city || item.country || t('shortlist.locationNotSet')}
                        </Text>
                      </View>
                      ...
                      {item.age && (
                        <View className="mt-2 bg-pink-50 px-2 py-1 rounded-full self-start">
                          <Text className="text-pink-600 text-xs font-medium">{item.age} years</Text>
                        </View>
                      )}
                      ...
                      <View className="flex-row gap-2 mt-3">
                        ...
                        >
                          ...
                          <Text className="text-slate-700 text-xs font-semibold ml-1">{t('common.view')}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          onPress={() => handleSendInterest(item.user_id, item.name || '')}
                          className="flex-1 bg-pink-500 py-2 rounded-lg items-center flex-row justify-center"
                        >
                          <HeartIcon size={14} color="white" />
                          <Text className="text-white text-xs font-semibold ml-1">{t('common.interest')}</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                  </TouchableOpacity>
