...
const TwoFactorSetupModal: React.FC<TwoFactorSetupModalProps> = ({
    ...
    const [setupData, setSetupData] = useState<{ qrCode?: string; manualKey?: string } | null>(
        null,
    );
    ...
    const handleVerify = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const response = await api.post('/member/account/2fa/verify', {
        ...
        <div className="space-y-4 animate-in slide-in-from-right">
            <div className="text-center mb-6">
                <h3 className="text-xl font-bold text-slate-900">
                    {t('modals.twoFactor.chooseMethod')}
                </h3>
                ...
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-bold text-slate-900">
                            {t('modals.twoFactor.authenticatorApp')}
                        </h4>
                        <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                            {t('modals.twoFactor.recommended')}
                        </span>
                    </div>
                    <p className="text-xs text-slate-500">
                        {t('modals.twoFactor.authenticatorDesc')}
                    </p>
                </div>
            </div>
            ...
            >
                ...
                <div>
                    <h4 className="font-bold text-slate-900 mb-1">
                        {t('modals.twoFactor.emailAddress')}
                    </h4>
    ...
    const renderConfig = () => (
        <div className="space-y-6 animate-in slide-in-from-right">
            <div className="text-center">
                <h3 className="text-xl font-bold text-slate-900">
                    {method === 'app'
                        ? t('modals.twoFactor.setupAuthenticator')
                        : t('modals.twoFactor.verifyContact')}
                </h3>
                <p className="text-slate-500 text-sm">
                    {method === 'app'
                        ? t('modals.twoFactor.scanQR')
                        : t('modals.twoFactor.codeSentToContact', { method: 'email' })}
                </p>
            </div>
            ...
            {method === 'app' ? (
                <div className="flex flex-col items-center gap-6">
                    <div className="p-4 bg-white border-2 border-slate-200 rounded-xl shadow-sm">
                        {setupData?.qrCode ? (
                            <img src={setupData.qrCode} alt="QR Code" className="size-40" />
                        ) : (
                    ...
                    {setupData?.manualKey && (
                        <div className="w-full bg-slate-50 p-3 rounded-lg border border-slate-200 flex justify-between items-center">
                            ...
                            <button
                                onClick={() => handleCopy(setupData.manualKey || '')}
            ...
            <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-2">
                    {t('modals.twoFactor.enterVerificationCode')}
                </label>
                <input
                    type="text"
            ...
            >
                {loading ? (
                    ...
                    t('modals.twoFactor.verifyAndContinue')
                )}
            </button>
        </div>
    );
    ...
    const renderRecovery = () => (
        <div className="space-y-6 animate-in slide-in-from-right">
            <div className="text-center">
                <h3 className="text-xl font-bold text-slate-900">
                    {t('modals.twoFactor.regenerateTitle')}
                </h3>
            ...
            >
                {loading ? (
                    ...
                    t('modals.twoFactor.generateCodes')
                )}
            </button>
        </div>
    );
    ...
    const renderBackup = () => (
        <div className="space-y-6 animate-in slide-in-from-right">
            <div className="text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900">
                    {t('modals.twoFactor.saveBackupCodes')}
                </h3>
            ...
            <div className="flex gap-3">
                ...
                <button
                    onClick={() => handleDownload(recoveryCodes.join('\n'))}
                    className="flex-1 py-2 border border-slate-200 rounded-lg text-sm font-bold hover:bg-slate-50 flex items-center justify-center gap-2"
                >
            ...
            <label className="flex items-start gap-3 p-4 bg-orange-50 border border-orange-100 rounded-xl cursor-pointer">
                ...
                <span className="text-sm text-orange-900 font-medium">
                    {t('modals.twoFactor.savedConfirm')}
                </span>
            </label>
    ...
    const renderSuccess = () => (
        <div className="text-center py-8 animate-in zoom-in-95">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-2">
                {t('modals.twoFactor.twoFactorEnabled')}
            </h3>
            ...
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-slate-50">
                    <div className="flex items-center gap-2">
                        ...
                        {isRecoveryMode && <Lock className="text-primary" size={20} />}
                        <h2 className="text-lg font-bold text-slate-900">
                            {isRecoveryMode
                                ? t('modals.twoFactor.recoveryCodes')
                                : t('modals.twoFactor.setup2FA')}
                        </h2>
                    </div>
                    <button
                        onClick={onClose}
