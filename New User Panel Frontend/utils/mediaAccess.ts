...
const buildSnapshot = (profile: any, kind: 'profile_photo' | 'gallery_image'): MediaAccessSnapshot => {
  const prefix = kind === 'profile_photo' ? 'profile_photo' : 'gallery_image';
  const state = normalizeState(readFirst(profile, [
    `${prefix}RequestState`,
    ...
    kind === 'profile_photo' ? 'profile_view_resquest_status' : 'gallery_view_resquest_status',
  ]));
  const requested = truthy(readFirst(profile, [
  ...
  const approved = truthy(readFirst(profile, [
  ...
  const required = truthy(readFirst(profile, [
