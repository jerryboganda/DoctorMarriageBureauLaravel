...
class ChatController extends Controller
{
    ...
    public function get_old_messages(Request $request)
    {
        ...
        if (count($chats) > 0) {
            return ['messages' => view('frontend.member.messages.messages_part', compact('chats'))->render(),
                'first_message_id' => $chats->last()->id];
        } else {
            return ['messages' => '', 'first_message_id' => 0];
        }
    }
    ...
    public function chat_refresh($id)
    {
        ...
        return ['messages' => view('frontend.member.messages.messages_left_single', compact('chats'))->render(),
            'count' => count($chats)];
    }
