...
const MatchIntelligenceModal: React.FC<MatchIntelligenceModalProps> = ({
    profile,
    onClose,
    ...
    const hasExistingInterest =
        resolveInterestState(profile.interestStatus, profile.interestText) !== 'none';
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-slate-900">
                                {t('modals.matchIntelligence.title')}
                            </h2>
                            <p className="text-xs text-slate-500">
                                {data?.generatedAt || t('modals.matchIntelligence.analyzing')}
                            </p>
                        </div>
                    </div>
                    <button
                    ...
                    {loading ? (
                        <div className="h-64 flex flex-col items-center justify-center gap-4">
                            ...
                            <p className="text-slate-500 font-medium">
                                {t('modals.matchIntelligence.analyzingDeep')}
                            </p>
                        </div>
                    ) : error || !data ? (
                        <div className="h-64 flex flex-col items-center justify-center gap-4 text-center">
                            ...
                            <p className="text-slate-600 font-medium max-w-xs">
                                {error || t('modals.matchIntelligence.failedLoad')}
                            </p>
                            <button
                                onClick={fetchIntelligence}
                                className="text-primary font-bold hover:underline"
                            >
                                {t('modals.matchIntelligence.retry')}
                            </button>
                        </div>
                    ) : (
                                ...
                                <div className="relative size-40 mb-6">
                                    <svg className="size-full -rotate-90" viewBox="0 0 36 36">
                                        ...
                                        <path
                                            ...
                                            stroke="#d41173"
                                            strokeWidth="3"
                                            strokeDasharray={`${data.totalScore}, 100`}
                                        />
                                    </svg>
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                                        ...
                                        <span className="text-xs font-bold text-primary uppercase tracking-wider">
                                            {t('modals.matchIntelligence.compatible')}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    {data.categories.map((cat, idx) => (
                                        <div key={idx} className="w-full">
                                            ...
                                            <div className="text-[10px] text-slate-400 text-left mt-0.5">
                                                {t('modals.matchIntelligence.weight', {
                                                    value: cat.weight,
                                                })}
                                            </div>
                                        </div>
                            ...
                            <div className="md:col-span-2 space-y-8">
                                ...
                                <div className="bg-slate-50 rounded-xl p-5 border border-slate-100">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <h3 className="font-bold text-slate-900 text-sm">
                                            {t('modals.matchIntelligence.mutualFit')}
                                        </h3>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="flex-1 text-center">
                                            <div className="text-2xl font-bold text-slate-900">
                                                {data.mutualFit.youMeetThem}%
                                            </div>
                                            <div className="text-xs text-slate-500">
                                                {t('modals.matchIntelligence.youMeetThem')}
                                            </div>
                                        </div>
                                        <div className="h-8 w-px bg-slate-200"></div>
                                        <div className="flex-1 text-center">
                                            <div className="text-2xl font-bold text-slate-900">
                                                {data.mutualFit.theyMeetYou}%
                                            </div>
                                            <div className="text-xs text-slate-500">
                                                {t('modals.matchIntelligence.theyMeetYou')}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        >
                                            {showFriction
                                                ? t('modals.matchIntelligence.hide')
                                                : t('modals.matchIntelligence.viewOptIn')}
                                        </button>
                                    </div>
                                    ...
                                    <div className="bg-purple-50 border border-purple-100 rounded-xl p-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <h3 className="font-bold text-purple-900 text-sm">
                                                {t('modals.matchIntelligence.matchmakerNote')}
                                            </h3>
                                        </div>
                ...
                {sendError && <div className="px-6 pb-2 text-xs text-red-600">{sendError}</div>}
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 text-slate-600 font-bold text-sm hover:text-slate-900"
                    >
                        {t('modals.matchIntelligence.close')}
                    </button>
                    <button
                    ...
                    >
                        {sending
                            ? t('modals.matchIntelligence.sending')
                            : sent
                              ? t('modals.matchIntelligence.proposalSent')
                              : t('modals.matchIntelligence.sendProposal')}
                    </button>
                </div>
            </div>
        </div>
