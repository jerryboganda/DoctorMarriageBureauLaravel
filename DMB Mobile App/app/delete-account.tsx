...
import { 
  ChevronLeftIcon, TrashIcon, AlertTriangleIcon, 
  CheckCircleIcon, XCircleIcon, LockIcon, UserXIcon
...
const DeleteAccountScreen = () => {
  ...
  const handleDeleteAccount = async () => {
    ...
    Alert.alert(
      ...
      t('deleteAccount.finalConfirmMsg'),
      [
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (response.data.result) {
                ...
                Alert.alert(t('common.error'), response.data.message || t('deleteAccount.deleteFailed'));
              }
            } catch (error: any) {
              if (error.response?.status === 401) {
                Alert.alert(t('deleteAccount.wrongPassword'), t('deleteAccount.wrongPasswordMsg'));
              } else {
                Alert.alert(t('common.error'), error.response?.data?.message || t('deleteAccount.deleteFailed'));
              }
            } finally {
              setLoading(false);
            }
      ...
      >
        ...
        <View className="px-4 pb-6">
            ...
            {[1, 2, 3].map((s) => (
              <React.Fragment key={s}>
                <View className={`w-8 h-8 rounded-full items-center justify-center ${
                  ...
                  <Text className={`font-bold ${step >= s ? 'text-red-700' : 'text-white'}`}>
                    {s}
                  </Text>
                </View>
                {s < 3 && (
                  <View className={`w-12 h-1 mx-1 rounded ${
                    step > s ? 'bg-white' : 'bg-white/30'
                  }`} />
                )}
              </React.Fragment>
          ...
          <View className="flex-row justify-between mt-2 px-2">
            <Text className="text-white/70 text-xs">{t('deleteAccount.stepReason')}</Text>
            <Text className="text-white/70 text-xs">{t('deleteAccount.stepReview')}</Text>
            <Text className="text-white/70 text-xs">{t('deleteAccount.stepConfirm')}</Text>
          </View>
        </View>
      </LinearGradient>
      ...
      <ScrollView className="flex-1 px-4 -mt-4" showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            {deleteReasonKeys.map((reason) => (
              ...
              >
                ...
                <Text className={`ml-3 ${
                  selectedReason === reason.id ? 'text-red-700 font-semibold' : 'text-slate-700'
                }`}>
                  {t(reason.labelKey)}
                </Text>
              </TouchableOpacity>
        ...
        {step === 2 && (
          ...
          >
            ...
            <View className="bg-amber-50 rounded-xl p-4">
              <Text className="text-amber-800 font-semibold mb-2">{t('deleteAccount.considerOptions')}:</Text>
        ...
        {step === 3 && (
          ...
          >
            ...
            >
              <LinearGradient
                colors={loading || !password || confirmText.toLowerCase() !== 'delete' 
                  ? ['#94a3b8', '#94a3b8'] 
                  : ['#991b1b', '#dc2626']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
        ...
        {step === 1 && (
          ...
          >
            <Text className="text-slate-500 font-semibold">{t('deleteAccount.changedMind')}</Text>
          </TouchableOpacity>
        )}
