...
import { 
  LockIcon, ChevronLeftIcon, EyeIcon, EyeOffIcon, CheckCircleIcon, ShieldIcon
...
const ChangePasswordScreen = () => {
  ...
  const passwordRequirements = [
    ...
    { label: t('changePassword.reqSpecial'), check: (p: string) => /[!@#$%^&*(),.?":{}|<>]/.test(p) },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.result) {
        ...
        Alert.alert(t('common.error'), response.data.message || t('changePassword.changeFailed'));
      }
    } catch (error: any) {
      Alert.alert(t('common.error'), error.response?.data?.message || t('changePassword.changeFailed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <View className="flex-1 bg-slate-50">
      ...
      >
        <View className="flex-row items-center justify-between px-4 py-4">
          ...
          <Text className="text-xl font-bold text-white">{t('changePassword.title')}</Text>
      ...
      <ScrollView className="flex-1 px-4 -mt-4" showsVerticalScrollIndicator={false}>
        ...
        >
          {/* Current Password */}
          <View className="mb-4">
            <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('changePassword.currentPassword')}</Text>
            <View className="flex-row items-center bg-slate-100 rounded-xl px-4">
              <TextInput
                className="flex-1 py-4 text-base text-slate-900"
                value={formData.current_password}
                onChangeText={(v) => setFormData(prev => ({ ...prev, current_password: v }))}
          ...
          {/* New Password */}
          <View className="mb-4">
            <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('changePassword.newPassword')}</Text>
            <View className="flex-row items-center bg-slate-100 rounded-xl px-4">
              <TextInput
                className="flex-1 py-4 text-base text-slate-900"
                value={formData.new_password}
                onChangeText={(v) => setFormData(prev => ({ ...prev, new_password: v }))}
            ...
            {passwordRequirements.map((req, index) => {
              ...
              return (
                <View key={index} className="flex-row items-center mb-1.5 last:mb-0">
                  ...
                  <Text className={`ml-2 text-sm ${passed ? 'text-emerald-600' : 'text-slate-500'}`}>
                    {req.label}
                  </Text>
                </View>
              );
          ...
          {/* Confirm Password */}
          <View className="mb-6">
            <Text className="text-xs font-bold text-slate-500 uppercase mb-2">{t('changePassword.confirmNewPassword')}</Text>
            <View className="flex-row items-center bg-slate-100 rounded-xl px-4">
              <TextInput
                className="flex-1 py-4 text-base text-slate-900"
                value={formData.confirm_password}
                onChangeText={(v) => setFormData(prev => ({ ...prev, confirm_password: v }))}
            ...
            {formData.confirm_password && formData.new_password !== formData.confirm_password && (
              <Text className="text-red-500 text-xs mt-1">{t('changePassword.noMatch')}</Text>
            )}
          </View>
          ...
          >
            ...
            >
              {loading ? (
                ...
                <>
                  <LockIcon size={20} color="white" />
                  <Text className="text-white font-bold text-lg ml-2">{t('changePassword.updatePassword')}</Text>
                </>
              )}
            </LinearGradient>
          </TouchableOpacity>
