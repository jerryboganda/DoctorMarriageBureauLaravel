...
import { View, Text, TouchableOpacity, KeyboardAvoidingView, Platform, ScrollView, Alert, Image, ActivityIndicator } from 'react-native';
...
export default function ForgotPassword() {
  ...
  const handleResetRequest = async () => {
    ...
    try {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      const message = error.response?.data?.message || error.message || t('auth.forgotPassword.sendError');
    ...
    return (
      <View className="flex-1 bg-white">
        ...
        <View className="flex-1 items-center justify-center px-6">
          ...
          <Text className="text-2xl font-bold text-slate-900 text-center mb-2">{t('auth.forgotPassword.checkMethod', { method: method === 'email' ? t('common.email') : t('common.phone') })}</Text>
          ...
          <View className="flex-1 px-8" style={{ marginTop: insets.top + 20, paddingBottom: 40 }}>
            ...
            >
              ...
              {method && (
                ...
                >
                  ...
                  <Text className="text-xs font-semibold text-blue-600">
                    {method === 'email' ? t('common.email') : t('common.phone')} {t('common.verification')}
                  </Text>
                </MotiView>
              )}
              ...
              <Input
                ...
                icon={method === 'email' ? <MailIcon size={22} color="#64748b" /> : (method === 'phone' ? <PhoneIcon size={22} color="#64748b" /> : <MailIcon size={22} color="#64748b" />)}
              ...
              <Button
                variant="primary"
                title={loading ? t('common.sending') : t('auth.forgotPassword.sendResetCode')}
                fullWidth
            ...
            >
              <Text className="text-slate-400 font-medium">
                {t('auth.forgotPassword.rememberedIt')} <Text className="text-blue-600 font-bold">{t('auth.forgotPassword.signIn')}</Text>
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
