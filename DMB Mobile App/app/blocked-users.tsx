                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            if (response.data.result) {
                                ...
                                Alert.alert(
                                    t('common.error'),
                                    response.data.message || t('blockedUsers.unblockFailed'),
                                );
                            }
                        } catch (error: any) {
                            Alert.alert(
                                t('common.error'),
                                error.response?.data?.message || t('blockedUsers.unblockFailed'),
                            );
                        } finally {
                ...
                <View className="items-center pb-6">
                    ...
                    <View className="mt-3 bg-white/20 px-4 py-2 rounded-full">
                        <Text className="text-white font-semibold">
                            {t('blockedUsers.blockedCount', { count: blockedUsers.length })}
                        </Text>
                    </View>
                </View>
            </LinearGradient>
                        ...
                        >
                            <View className="bg-white rounded-2xl p-4 shadow-sm flex-row items-center">
                                ...
                                {item.blocked_user.photo ? (
                                    <Image
                                        source={{
                                            uri: getProfileImageUrl(item.blocked_user.photo),
                                        }}
