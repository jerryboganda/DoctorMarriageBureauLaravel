...
import { View, Text, TouchableOpacity, ScrollView, Modal, Pressable, Dimensions } from 'react-native';
      ...
      <View className="flex-1">
        ...
        >
          ...
          <View className="flex-row items-center justify-between mb-6">
            ...
            >
              <Text className="text-2xl font-black text-slate-800 mr-2">{MONTHS[viewDate.getMonth()]}</Text>
              <Text className="text-2xl font-black text-brand-blue">{viewDate.getFullYear()}</Text>
            ...
            {view === 'calendar' && (
              <View className="flex-row gap-2">
                <TouchableOpacity onPress={() => changeMonth(-1)} className="p-2 bg-slate-100 rounded-full">
            ...
            {view === 'calendar' ? (
              <View>
                <View className="flex-row mb-2">
                  {DAYS.map(day => (
                    <Text key={day} className="flex-1 text-center text-xs font-bold text-slate-400">{day}</Text>
                  ))}
                </View>
                <View className="flex-row flex-wrap">
                  {blanks.map(i => <View key={`blank-${i}`} className="w-[14.28%] h-12" />)}
                  {days.map(day => {
                    ...
                    return (
                      ...
                      >
                        <Text className={`text-base font-semibold ${isSelected ? 'text-white' : 'text-slate-700'}`}>{day}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
              ...
              <ScrollView className="h-[300px]" showsVerticalScrollIndicator={false}>
                <View className="flex-row flex-wrap gap-2">
                  {years.map(year => (
                    ...
                    >
                      <Text className={`font-bold text-lg ${viewDate.getFullYear() === year ? 'text-white' : 'text-slate-600'}`}>{year}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </ScrollView>
