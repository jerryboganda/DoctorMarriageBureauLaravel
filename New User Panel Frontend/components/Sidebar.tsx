...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    useEffect(() => {
        ...
        const fetchCounts = async () => {
            ...
            try {
                ...
                const discoveryTask = shouldFetchDiscovery
                    ? api.get('/discovery')
                ...
                const proposals =
                    interestsRes?.data?.meta?.total ?? interestsRes?.data?.data?.length ?? 0;
    ...
    return (
        <aside className="w-[280px] h-full flex flex-col bg-white border-r border-slate-200 shrink-0 relative shadow-xl lg:shadow-none">
            ...
            <nav
                ref={navRef}
                className="flex-1 flex flex-col px-4 gap-1 py-4 overflow-y-auto scrollbar-hide relative"
            >
                <div className="space-y-1">
                    <NavItem
                        icon={<Compass size={20} />}
                        label={t('nav.exploreProfiles')}
                        active={currentView === 'discovery'}
                        onClick={() => onNavigate('discovery')}
                    />
                    <NavItem
                        icon={<UserCheck size={20} />}
                        label={t('nav.matchmakerProposals')}
                        active={currentView === 'agent_picks'}
                        onClick={() => onNavigate('agent_picks')}
                        badge={counts.agentPicks || undefined}
                    />
                    <NavItem
                        icon={<Heart size={20} />}
            ...
            <div className="shrink-0 p-4 bg-white border-t border-slate-100 space-y-1">
                <LanguageToggle
                    compact={false}
                    className="w-full justify-start gap-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-600 hover:bg-amber-50 hover:text-amber-700 transition-all"
                />
                <button
                    onClick={() => onNavigate('signout')}
                    className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all w-full font-bold text-sm"
                >
...
const NavItem = ({ icon, label, sublabel, active, badge, onClick }: any) => (
    ...
    >
        ...
        <span className="flex-1 text-left">
            {label}
            {sublabel && (
                <span className="ml-1.5 text-[10px] font-semibold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full">
                    {sublabel}
                </span>
            )}
        </span>
        {badge && (
            <span className="text-[10px] bg-primary text-white font-bold px-1.5 py-0.5 rounded-md">
                {badge}
            </span>
        )}
    </motion.button>
