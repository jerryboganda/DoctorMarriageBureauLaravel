...
import { View, Text, TextInput, TouchableOpacity, Image, RefreshControl, Dimensions, Alert } from 'react-native';
...
import { SearchIcon, FilterIcon, HeartIcon, MapPinIcon, StarIcon, SendIcon, ChevronRightIcon, XIcon, SparklesIcon } from '../../components/Icons';
...
export default function DiscoveryTab() {
  ...
  const handleSendInterest = async (profile: ProfileMatch) => {
    ...
    try {
      ...
      Alert.alert(t('discovery.interestSentTitle'), t('discovery.interestSentMsg', { name: profile.name }));
    } catch (error: any) {
      Alert.alert(t('discovery.errorTitle'), error.response?.data?.message || t('discovery.interestFailedMsg'));
    } finally {
      ...
      >
        ...
        <View className="relative">
          ...
          >
            ...
            <Text className="text-[10px] font-bold text-blue-700">{item.matchPercentage}%</Text>
          </TouchableOpacity>

          {item.isAgentPick && (
            <View className="absolute top-2 left-2 bg-amber-500 px-2 py-1 rounded-full">
              <Text className="text-[8px] font-bold text-white uppercase">{t('discovery.agentPick')}</Text>
            </View>
          )}
        </View>
        ...
        <View className="p-3">
          ...
          <Text className="text-blue-600 text-xs mt-0.5" numberOfLines={1}>{item.specialty || 'Member'}</Text>
          ...
          >
            ...
            <Text className="text-white text-xs font-bold">
              {sendingInterest[item.id] ? t('discovery.sending') : t('discovery.sendInterest')}
            </Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
  ...
  const EmptyState = () => (
    <View className="flex-1 items-center justify-center px-8 py-20">
      ...
      <Text className="text-lg font-bold text-slate-900 text-center mb-2">{t('discovery.noProfilesTitle')}</Text>
      <Text className="text-slate-500 text-center text-sm">{t('discovery.noProfilesDesc')}</Text>
    </View>
  );
      ...
      <View className="px-4 pb-3 bg-white/80 border-b border-slate-100" style={{ paddingTop: insets.top + 12 }}>
        ...
        <View className="flex-row bg-slate-100 p-1 rounded-full">
          {[
            ...
            >
              <Text className={`text-xs font-bold ${activeTab === tab.key ? 'text-blue-700' : 'text-slate-500'}`}>
                {tab.label}
              </Text>
            </TouchableOpacity>
          ))}
