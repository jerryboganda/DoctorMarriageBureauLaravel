...
const NotificationsView: React.FC<NotificationsViewProps> = ({ onNavigate, onOpenProfile, refreshVersion, onDataChanged }) => {
  ...
  const updateNotification = (notificationId: string, updater: (item: NotificationItem) => NotificationItem) => {
    setNotifications((current) => current.map((item) => (
      item.notification_id === notificationId ? updater(item) : item
    )));
  ...
  const markAllRead = async () => {
    try {
      ...
      setSelectedNotification((current) => current ? { ...current, read_at: 'read', is_read: true } : null);
      onDataChanged?.();
    } catch (error) {
            ...
            displayedNotifications.map((notification) => {
              ...
              return (
                <div
                  ...
                  onClick={() => markNotificationRead(notification.notification_id, false)}
                  ...
                  <div className="flex items-start gap-4">
                    <div className="relative shrink-0">
                      <img
                        ...
                        alt={notification.title || notification.type || 'Notification'}
                        className="size-12 rounded-2xl object-cover md:size-14"
                      />
                      <div className={`absolute -bottom-1 -right-1 flex size-6 items-center justify-center rounded-lg border-2 border-white text-white ${notification.type === 'express_interest' ? 'bg-pink-500' : notification.type === 'gallery_image_view' ? 'bg-primary' : 'bg-slate-400'}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex items-start justify-between">
                        <h3 className={`truncate text-slate-900 ${isUnread ? 'font-bold' : 'font-medium'}`}>
                          {notification.title}
                        </h3>
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        ...
                        {isUnread ? <span className="size-2 rounded-full bg-primary"></span> : null}
                      </div>
                    </div>
                  </div>
                </div>
