...
import { XIcon, CameraIcon, CheckCircleIcon, ShieldIcon, FileBadgeIcon } from './Icons';
            ...
            >
                ...
                <div className="flex-1 p-6 flex flex-col items-center justify-center text-center">
                    <AnimatePresence mode="wait">
                        ...
                        {/* STEP 0: INTRO */}
                        {step === 0 && (
                            <motion.div key="intro" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col items-center">
                                ...
                                <p className="text-slate-500 text-sm mb-8 leading-relaxed">
                                    To maintain a trusted community, we require a government-issued ID verification. Your data is encrypted and deleted after verification.
                                </p>
                        ...
                        {step === 1 && (
                            <motion.div key="scan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center w-full">
                                <h3 className="font-bold text-slate-900 mb-6">Scanning Document...</h3>
                                ...
                                <p className="text-sm text-slate-400 mt-6 animate-pulse">Align your ID within the frame</p>
                            </motion.div>
                        )}
                        ...
                        {step === 2 && (
                            <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center">
                                ...
                                <h2 className="text-2xl font-black text-slate-900 mb-2">Verification Complete</h2>
                                <p className="text-slate-500 text-sm mb-8">
                                    Thank you! Your identity has been verified securely. You now have the verified badge.
                                </p>
                                <Button onClick={onClose} className="bg-green-600 hover:bg-green-700 shadow-green-500/30">Continue</Button>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
